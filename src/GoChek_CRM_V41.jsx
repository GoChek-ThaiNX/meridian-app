      ...
      },
      {
        ...
        content: [
          { type: "table", headers: ["Phiên bản", "Tính năng chính"], rows: [
            ...
            ["V38i", "🔧 Fix lỗi V38h: OB chuyển sang model TT × NCC (mỗi OB ghi rõ TT nào nợ NCC nào). Hard migration xóa OB cũ. Đồng bộ 2 chiều: 1 OB tự động xuất hiện cả tab NCC + tab TT + Dashboard + báo cáo Excel."],
          ]},
        ],
      },
    ],
...
// ============================================================
const POForm = ({ initial, factories, products, shipments, data, user, onRenameId, onSave, onClose }) => {
  const [form, setForm] = useState(initial ? {
    ...initial,
  ...
  // v38f: Hỗ trợ đổi mã PO (Phương án D — chỉ khi entity sạch, mọi user có quyền edit)
  ...
  // v38i fix: Bug từ V38f — quên định nghĩa canShowRename + currentId.
  // canShowRename: hiển thị nút "Đổi mã" khi đang sửa + có quyền + entity còn cho rename
  // currentId: mã PO hiện tại (lấy từ initial.id khi sửa, hoặc rỗng khi tạo mới)
...
const POs = ({ pos, factories, products, shipments, markets = [], settings, onAdd, onEdit, onDelete, onHardDelete, onRenameId, data, onConfirm, user }) => {
  const [modal, setModal] = useState(null);
  ...
  return (
    <div>
      ...
      {modal?.type === "new" && <POForm factories={factories} products={products} shipments={shipments} data={data} user={user} onSave={f => {
...
// ============================================================
const ShipmentForm = ({ initial, pos, shipments: allShipments, factories, products, markets, carriers, settings, onSave, onCreateWarehouse, onClose, data, feePayments = [], stockMovements = [] }) => {
  const marketNames = getMarketNames(markets);
...
const Shipments = ({ shipments, pos, factories, products, feePayments, markets, carriers, settings, onAdd, onEdit, onDelete, onHardDelete, onRenameId, data, onCreateWarehouse, user }) => {
  const marketNames = getMarketNames(markets);
  const [modal, setModal] = useState(null);
...
const OpeningBalances = ({ openingBalances, factories, markets = [], settings, onAdd, onEdit, onDelete, onMigrationDismiss, migrationFlag, oldOBCount, user }) => {
  const [modal, setModal] = useState(null);
        ...
        <div className="alert alert-warn" style={{ marginBottom: 16, display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 12 }}>
          <div>
            ...
            <div style={{ fontSize: 12 }}>
              Đã xóa <b>{oldOBCount}</b> công nợ đầu kỳ cũ. V38h có lỗi logic (TT đang nợ "không ai") nên V38i chuyển sang model <b>TT × NCC</b>: mỗi OB phải ghi rõ TT nào nợ NCC nào.
              <br />Vui lòng nhập lại OB theo schema mới (bấm "+ Thêm công nợ đầu kỳ").
            </div>
          </div>
...
export default function App() {
  ...
  // v38i: Cờ banner cảnh báo migration OB (chỉ hiện 1 lần khi user mở tab)
  const [migrationDismissed, setMigrationDismissed] = useState(false);
  const onMigrationDismiss = useCallback(async () => {
    setMigrationDismissed(true);
    // Lưu vào storage để không hiện lại lần sau
    setData(d => {
      const next = { ...d, _v38i_migrated: false }; // tắt flag
      saveAll(next);
      return next;
    });
  }, []);
  ...
  useEffect(() => {
    (async () => {
      ...
      if (saved) {
        ...
        // v38i: HARD MIGRATION OB — XÓA SẠCH OB cũ.
        // Lý do: V38h có lỗi logic vô lý (OB market tăng nợ TT nhưng không tăng nợ NCC).
        // V38i chuyển sang model TT × NCC: mỗi OB BẮT BUỘC có cả market + factoryId.
        // Vì migration tự động không thể đoán được TT-NCC pair từ data cũ → user phải nhập lại.
        // Chị đã chốt: data cũ chỉ là test, hard delete OK.
  ...
  const onEdit = (key, id, updates) => {
    ...
    if (isRenaming) {
      ...
      delete cleanUpdates._renamedFrom;
      ...
      <div style={{ display: "flex", minHeight: "100vh", background: C.bg }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
          ...
          <div style={{ padding: 28, flex: 1 }}>
            ...
            {tab === "pos" && <POs pos={view.pos} factories={view.factories} products={view.products} shipments={view.shipments} markets={view.markets} settings={view.settings} onAdd={onAdd} onEdit={onEdit} onDelete={onDelete} onHardDelete={onHardDelete} onRenameId={onRenameId} data={data} onConfirm={setConfirmDialog} user={user} />}
            {tab === "shipments" && <Shipments shipments={view.shipments} pos={view.pos} factories={view.factories} products={view.products} feePayments={view.feePayments} markets={view.markets} carriers={view.carriers} settings={view.settings} onAdd={onAdd} onEdit={onEdit} onDelete={onDelete} onHardDelete={onHardDelete} onRenameId={onRenameId} data={data} onCreateWarehouse={onCreateWarehouse} user={user} />}
            ...
            {tab === "market_debts" && <MarketDebts pos={view.pos} shipments={view.shipments} payments={view.payments} factories={view.factories} markets={view.markets} settings={view.settings} products={view.products} warranties={view.warranties} openingBalances={view.openingBalances} user={user} />}
            {tab === "opening_balance" && <OpeningBalances openingBalances={view.openingBalances} factories={view.factories} markets={view.markets} settings={view.settings} onAdd={onAdd} onEdit={onEdit} onDelete={onDelete} migrationFlag={data._v38i_migrated && !migrationDismissed} oldOBCount={data._v38i_oldOBCount || 0} onMigrationDismiss={onMigrationDismiss} user={user} />}
