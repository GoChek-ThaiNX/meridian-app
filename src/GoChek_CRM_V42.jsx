...
// STORAGE & UTILS
// ============================================================
// v42: Storage được chuyển sang s3Storage.js (S3 + localStorage cache + memory fallback)
...
const calcAtFactoryQty = (productId, pos, shipments) => {
  ...
  shipments.forEach(s => {
    if (s.status === "Hủy" || s.status === "Nháp") return;
    (s.items || []).forEach(it => {
      const po = pos.find(p => p.id === it.poId);
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      ...
      // v42: Chuyển sang s3Storage.js — loadAll() tự GET từ S3 → fallback localStorage → memory
      ...
      if (!saved) {
        ...
        if (migratedFrom) console.info(`[GC CRM] Migrating data from ${migratedFrom} → v42 (S3)`);
      }
      ...
      if (saved) {
        ...
        if (v38jProductMigrated > 0) {
          console.info(`[GC CRM v38j] Đã migrate ${v38jProductMigrated} sản phẩm với schema mới.`);
        }
        ...
        setData(d => ({ ...d, ...saved }));
        // v42: Save to S3 + localStorage via s3Storage
        await saveAll(saved);
      } else {
  ...
  // v42: Auto sync S3 — Push/Pull mỗi 2 phút + flush khi đóng tab
