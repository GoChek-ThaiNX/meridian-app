...
// ============================================================
// GoChek Factory CRM — Phiên bản V38e
// ============================================================
...
// ============================================================
// v38d: HARD DELETE HELPERS — Kiểm tra điều kiện xóa cứng (Phương án A)
// ============================================================
...
const Products = ({ products, pos, shipments, factories, markets = [], settings, onAdd, onEdit, onDelete, onSaveSettings, onImportProducts, user }) => {
...
const POs = ({ pos, factories, products, shipments, markets = [], settings, onAdd, onEdit, onDelete, onHardDelete, data, onConfirm, user }) => {
  const [modal, setModal] = useState(null);
...
const Shipments = ({ shipments, pos, factories, products, feePayments, markets, carriers, settings, onAdd, onEdit, onDelete, onHardDelete, data, onCreateWarehouse, user }) => {
  const marketNames = getMarketNames(markets);
  const [modal, setModal] = useState(null);
...
const OpeningBalances = ({ openingBalances, factories, settings, onAdd, onEdit, onDelete, user }) => {
  ...
  // v38b: Sort theo date desc (mới nhất trên đầu, tie-break ID)
  const filtered = sortByDateDesc(
    (filterFactory ? openingBalances.filter(o => o.factoryId === filterFactory) : openingBalances)
      ...
      <div style={{ display: "flex", minHeight: "100vh", background: C.bg }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
          ...
          <div style={{ padding: 28, flex: 1 }}>
            ...
            {tab === "products" && <Products products={view.products} pos={view.pos} shipments={view.shipments} factories={view.factories} markets={view.markets} settings={view.settings} onAdd={onAdd} onEdit={onEdit} onDelete={onDelete} onSaveSettings={onSaveSettings} onImportProducts={onImportProducts} user={user} />}
            {tab === "pos" && <POs pos={view.pos} factories={view.factories} products={view.products} shipments={view.shipments} markets={view.markets} settings={view.settings} onAdd={onAdd} onEdit={onEdit} onDelete={onDelete} onHardDelete={onHardDelete} data={data} onConfirm={setConfirmDialog} user={user} />}
            {tab === "shipments" && <Shipments shipments={view.shipments} pos={view.pos} factories={view.factories} products={view.products} feePayments={view.feePayments} markets={view.markets} carriers={view.carriers} settings={view.settings} onAdd={onAdd} onEdit={onEdit} onDelete={onDelete} onHardDelete={onHardDelete} data={data} onCreateWarehouse={onCreateWarehouse} user={user} />}
