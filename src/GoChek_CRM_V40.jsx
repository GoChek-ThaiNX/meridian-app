...
// ============================================================
// GoChek Factory CRM — Phiên bản V38g
// ============================================================
    ...
    <Modal
      ...
      {renameDlg && (
        <RenameIdDialog
          ...
          objectType="đơn giao hàng"
          duplicateExists={renameDlg.duplicateExists}
          onConfirm={() => {
...
const ConfirmArriveForm = ({ shipment, pos, products, markets, onSave, onClose }) => {
  ...
  return (
    <Modal title={`Xác nhận lô về kho — ${shipment.id}`} subtitle={`${shipment.market} · ${(shipment.items || []).length} dòng SP`}
      ...
      {/* v38f: Dialog xác nhận đổi mã đơn giao hàng — chỉ render khi handleSave detect ID đổi */}
      {renameDlg && (
        <RenameIdDialog
          title={`Đổi mã đơn giao hàng: ${renameDlg.oldId}`}
          ...
          onConfirm={(confirmedNewId) => {
            // confirmedNewId luôn = renameDlg.newId (do RenameIdDialog yêu cầu user gõ đúng)
            // Save form với newId đã set + flag _renamedFrom để App phân biệt
            onSave({ ...renameDlg.finalForm, id: confirmedNewId, _renamedFrom: renameDlg.oldId });
            setRenameDlg(null);
          }}
          onClose={() => setRenameDlg(null)}
        />
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      ...
      let saved = await storage.get("crm_data_v38");
      let migratedFrom = ""; // ghi nhận đã migrate từ phiên bản nào (debug/audit)
      if (!saved) {
        const LEGACY_VERSIONS = ["v34", "v31", "v23", "v22", "v21", "v20", "v19", "v18", "v17", "v16", "v15", "v14", "v13", "v12", "v11", "v10", "v9"];
      ...
      if (saved) {
        ...
        setData(d => ({ ...d, ...saved }));
        // v38: Save vào key mới
        await storage.set("crm_data_v38", saved);
      } else {
        ...
        setData(init);
        await storage.set("crm_data_v38", init);
      }
      ...
      try {
        ...
        if (sessionStr) {
          ...
          if (!session.userId || !session.expiresAt) {
            ...
            // Tìm user trong data
            const dataToCheck = saved || (await storage.get("crm_data_v38"));
  ...
  const save = useCallback(async (next) => {
    ...
    setData(synced);
    await storage.set("crm_data_v38", synced);
      ...
      <div style={{ display: "flex", minHeight: "100vh", background: C.bg }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
          ...
          <div style={{ padding: 28, flex: 1 }}>
            ...
            {tab === "products" && <Products products={data.products} pos={data.pos} shipments={data.shipments} factories={data.factories} markets={data.markets} settings={data.settings} onAdd={onAdd} onEdit={onEdit} onDelete={onDelete} onSaveSettings={onSaveSettings} onImportProducts={onImportProducts} user={user} />}
            {tab === "pos" && <POs pos={data.pos} factories={data.factories} products={data.products} shipments={data.shipments} markets={data.markets} settings={data.settings} onAdd={onAdd} onEdit={onEdit} onDelete={onDelete} onHardDelete={onHardDelete} onRenameId={onRenameId} data={data} onConfirm={setConfirmDialog} user={user} />}
            {tab === "shipments" && <Shipments shipments={data.shipments} pos={data.pos} factories={data.factories} products={data.products} feePayments={data.feePayments} markets={data.markets} carriers={data.carriers} settings={data.settings} onAdd={onAdd} onEdit={onEdit} onDelete={onDelete} onHardDelete={onHardDelete} onRenameId={onRenameId} data={data} onCreateWarehouse={onCreateWarehouse} user={user} />}
