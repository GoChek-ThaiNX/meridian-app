...
// ============================================================
// GoChek Factory CRM — Phiên bản V38
// ============================================================
...
const calcMarketBalance = (market, pos, shipments, payments, settings, warranties = [], products = []) => {
  ...
  // v18 + v38: Còn phải trả = Hàng đã ship − Đã thanh toán (stage 3) − Đang TT (stage 1+2) − Hàng đang BH
  const remain = totalReceivedCNY - totalPaidCNY - pendingPaidCNY - warrantyPendingCNY;
  const stillOwed = remain > 0 ? remain : 0;
  const creditFund = remain < 0 ? -remain : 0;
  return {
...
const exportMarketReport = async ({ marketName, isAllMarkets, pos, shipments, payments, factories, products, markets, warranties = [], dateFrom, dateTo, settings, exportedBy }) => {
  ...
  // v38: Còn phải trả = (1) − (3) − (3a) − (3.5) — Cách B
  const stillOwed = Math.max(0, grandShippedCNY - grandPaidCNY - grandPendingCNY - grandWarrantyCNY);
  const creditFund = Math.max(0, grandPaidCNY + grandPendingCNY + grandWarrantyCNY - grandShippedCNY);
  pushMoneyRow("(4) CÒN PHẢI TRẢ", stillOwed, "= (1) − (3) − (3a) − (3.5), tối thiểu 0", true);
...
const Products = ({ products, allProducts = [], pos, shipments, factories, markets = [], settings, onAdd, onEdit, onDelete, onRestore, onSaveSettings, onImportProducts, user }) => {
  ...
  const deletedProducts = useMemo(() => 
    allProducts.filter(p => p.deleted).sort((a, b) => new Date(b.deletedAt) - new Date(a.deletedAt)),
    [allProducts]
  );
      ...
      <SectionHeader title="Sản phẩm" subtitle="Quản lý SKU, tồn kho, đặt hàng, giao hàng"
        action={
          <div style={{ display: "flex", gap: 8 }}>
            <button className={`btn ${showDeleted ? "btn-primary" : "btn-ghost"}`} onClick={() => setShowDeleted(!showDeleted)}>
              {showDeleted ? "📂 Hiện tại" : `🗑️ Đã xóa (${deletedProducts.length})`}
            </button>
...
const POs = ({ pos, factories, products, shipments, markets = [], settings, onAdd, onEdit, onDelete, onConfirm, user }) => {
  const [modal, setModal] = useState(null);
...
// ============================================================
const Configuration = ({ data, view, user, onAdd, onEdit, onDelete, onSaveSettings, onCreateWarehouse }) => {
  ...
  return (
    <div>
      ...
      {activeSubtab === "general" && <Settings settings={view.settings} onSave={onSaveSettings} user={user} />}
      {activeSubtab === "factories" && <Factories factories={view.factories} pos={view.pos} shipments={view.shipments} settings={view.settings} onAdd={onAdd} onEdit={onEdit} onDelete={onDelete} user={user} />}
      {activeSubtab === "carriers" && <Carriers carriers={view.carriers} shipments={view.shipments} feePayments={view.feePayments} settings={view.settings} onAdd={onAdd} onEdit={onEdit} onDelete={onDelete} user={user} />}
      {activeSubtab === "markets" && <Markets markets={view.markets} shipments={view.shipments} payments={view.payments} onAdd={onAdd} onEdit={onEdit} onDelete={onDelete} user={user} />}
...
const OpeningBalances = ({ openingBalances, factories, settings, onAdd, onEdit, onDelete, user }) => {
  ...
  const summary = useMemo(() => {
    ...
    // v33 fix: Loại trừ openingBalance đã hủy khỏi tổng cộng
    openingBalances.forEach(o => {
      if (!byFactory[o.factoryId]) return;
      if (o.status === "cancelled") return; // bỏ qua bản ghi đã hủy
...
export default function App() {
    ...
    (async () => {
      ...
      if (saved) {
        ...
        saved.stockMovements = (saved.stockMovements || []).map(mv => {
          ...
          if (mv.refType === "shipment_arrive" || mv.refType === "warranty_send" || mv.refType === "warranty_return") return mv;
        ...
        saved.payments = (saved.payments || []).map(p => {
          if (p.exchangeRate !== undefined && p.exchangeRate !== null) return p;
          ...
          return { ...p, exchangeRate: rate, amountInVND: Number(p.amount || 0) * rate };
        });

        saved.payments = (saved.payments || []).map(p => {
          if (p.paymentStage && Array.isArray(p.stageHistory)) return p;
          return {
            ...p,
        ...
        setData(d => ({ ...d, ...saved }));
        // v33: save initial to S3 if changed
        await saveAll(saved);
      } else {
        const init = {
      ...
      setLoaded(true);
      startAutoSync(() => dataRef.current, setData);
    })();
    return () => stopAutoSync();
  }, []);
  ...
  const onAdd = (key, item) => {
    ...
    addItem(data, key, item, newLog).then(setData);
  };
  const onEdit = (key, id, updates) => {
    ...
    editItem(data, key, id, updates, newLog).then(setData);
  };
  const onDelete = (key, id) => {
    ...
    setConfirmDialog({
      title: `Xóa ${label}?`,
      message: `Bạn có chắc chắn muốn xóa ${label} "${name}"?\n\nHành động này sẽ được đưa vào danh mục "Đã xóa" và có thể khôi phục sau này.`,
      confirmLabel: "Xóa",
      danger: true,
      onConfirm: () => {
        const newLog = addAuditLog(`delete_${actionLabel(key)}`, id);
        softDeleteItem(data, key, id, user?.username || "unknown", newLog).then(setData);
      },
    });
  };
  const onRestore = (key, id) => {
    const newLog = addAuditLog(`restore_${actionLabel(key)}`, id);
    editItem(data, key, id, { deleted: false, deletedAt: null, deletedBy: null }, newLog).then(setData);
  };
  const onSaveSettings = (newSettings) => {
    const newLog = addAuditLog("update_settings", "settings", newSettings);
    saveSettingsToS3(data, newSettings, newLog).then(setData);
  };
      ...
      <div style={{ display: "flex", minHeight: "100vh", background: C.bg }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
          ...
          <div style={{ padding: 28, flex: 1 }}>
            ...
            {tab === "products" && <Products products={view.products} allProducts={data.products} pos={view.pos} shipments={view.shipments} factories={view.factories} markets={view.markets} settings={view.settings} onAdd={onAdd} onEdit={onEdit} onDelete={onDelete} onRestore={onRestore} onSaveSettings={onSaveSettings} onImportProducts={onImportProducts} user={user} />}
            ...
            {tab === "configuration" && <Configuration data={data} view={view} user={user} onAdd={onAdd} onEdit={onEdit} onDelete={onDelete} onSaveSettings={onSaveSettings} onCreateWarehouse={onCreateWarehouse} />}
