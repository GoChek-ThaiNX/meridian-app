...
// STORAGE & UTILS
// ============================================================
// v44: Storage được chuyển sang s3Storage.js (S3 + localStorage cache + memory fallback)
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      ...
      // v44: Chuyển sang s3Storage.js — loadAll() tự GET từ S3 → fallback localStorage → memory
      ...
      if (!saved) {
        ...
        if (migratedFrom) console.info(`[GoChek CRM] Migrating data from ${migratedFrom} → v44 (S3)`);
      }
      ...
      if (saved) {
        ...
        // v44 FIX: Chỉ xóa OB schema CŨ (thiếu market HOẶC thiếu factoryId).
        ...
        setData(d => ({ ...d, ...saved }));
        // v44: Save to S3 + localStorage via s3Storage
        await saveAll(saved);
      } else {
  ...
  // v44: Auto sync S3 — Push/Pull mỗi 2 phút + flush khi đóng tab
  ...
  const handleLogout = async () => {
    ...
    // v44: Flush ngay lập tức lên S3 (cancel debounce pending + PUT)
    s3Flush(finalData);
