...
import {
  ...
  editItem,
  softDeleteItem,
  saveSettings as s3SaveSettings,
  saveMarkets as s3SaveMarkets,
  alive,
  startAutoSync,
  stopAutoSync,
  s3Flush,
...
// ============================================================
// GoChek Factory CRM — Phiên bản V33
// ============================================================
// CHANGELOG V33 (vs V32):
// 1. [Storage] Tích hợp s3Storage.js — toàn bộ persistence đi qua S3
//    - loadAll() thay cho storage.get("crm_data_v31") khi khởi động
//    - saveAll() thay cho storage.set() — debounced S3 PUT + localStorage cache
//    - addItem() cho onAdd → tránh race condition khi ghi đồng thời
//    - editItem() cho onEdit → merge updates + persist nguyên tử
//    - s3SaveSettings() cho onSaveSettings
//    - s3SaveMarkets() cho onCreateWarehouse
//    - s3Flush() khi logout → đảm bảo data không mất khi đóng tab
// 2. [Sync] startAutoSync() khởi động sau khi loaded — Push+Pull mỗi 5 phút
//    - Pull: nếu S3 mới hơn (nhiều audit log hơn) → cập nhật UI tự động
//    - Push: nếu local thay đổi → đẩy lên S3
//    - stopAutoSync() cleanup khi component unmount
// 3. [Ref] Thêm dataRef để autoSync đọc được data mới nhất mà không re-subscribe
// 4. [Migration] Legacy localStorage keys (v9–v23) vẫn được đọc khi S3 trống
//    (user chuyển từ phiên bản cũ lên V33 không mất data)
//
  ...
  business: {
    ...
    items: [
      ...
      },
      {
        ...
        content: [
          { type: "h", text: "Cách hệ thống tính:" },
          { type: "code", text: "Còn nợ NCC = Đầu kỳ + Σ giá trị Shipment đã về kho − Σ Payment đã thanh toán − BH treo công nợ" },
          ...
          { type: "list", items: [
            ...
            "💸 Payment loại MARKET_TO_FACTORY — giảm nợ",
            "🔧 Bảo hành treo công nợ — khi 3 lô bảo hành cùng NM ở trạng thái Pending → tạm treo công nợ thanh toán",
          ]},
      ...
      },
      {
        ...
        content: [
          ...
          { type: "h", text: "Cách tính:" },
          { type: "code", text: "TT đang nợ = Σ giá trị hàng đã giao về TT − Σ thanh toán TT đã chuyển về công ty" },
          { type: "tip", text: "Tab Công nợ thị trường giúp Liễu theo dõi từng TT đã chuyển tiền đủ chưa. Nếu TT có credit fund (đã chuyển dư) → có thể đối trừ với lô tiếp theo." },
...
// STORAGE & UTILS
// ============================================================
// v33: Storage được chuyển sang s3Storage.js (S3 + localStorage cache + memory fallback)
...
const calcDuePayments = (shipments, pos, factories, payments, products, settings, daysAhead = 14) => {
  ...
  factories.forEach(factory => {
    ...
    // Lấy tất cả lô đã về kho liên quan NCC này, sắp xếp theo arrive ascending (FIFO)
    const factoryShipments = shipments
      ...
      .map(s => {
        ...
        (s.items || []).forEach(i => {
          ...
          if (!poItem) return;
          const qty = Number(i.receivedQty ?? i.quantity ?? 0); // ưu tiên SL nhận thực tế
        ...
        const dueDate = arriveDate ? new Date(new Date(arriveDate).getTime() + paymentDays * 86400000).toISOString().slice(0, 10) : null;
        return { shipment: s, valueInFactoryCurrency, arriveDate, dueDate };
      })
...
const calcMarketBalance = (market, pos, shipments, payments, settings, warranties = [], products = []) => {
  ...
  // v20: Bỏ qua thanh toán đã hủy
  const totalPaidCNY = payments.filter(p => p.type === "MARKET_TO_FACTORY" && p.fromMarket === market && p.status !== "cancelled")
    .reduce((s, p) => {
  ...
  // v18: Còn phải trả = Hàng đã ship − Đã thanh toán − Hàng đang BH (treo)
  const remain = totalReceivedCNY - totalPaidCNY - warrantyPendingCNY;
  const stillOwed = remain > 0 ? remain : 0;
  const creditFund = remain < 0 ? -remain : 0;
  return { totalReceived: totalReceivedCNY, totalPaid: totalPaidCNY, warrantyPending: warrantyPendingCNY, stillOwed, creditFund };
};
...
const exportAccountingReport = async ({ factory, pos, shipments, payments, feePayments, openingBalances, products, carriers, markets, dateFrom, dateTo, settings, exportedBy }) => {
  ...
  // v15: Công nợ tính trên "đã ship" (khớp UI)
  ...
  pushMoneyRow("(5) Thanh toán: Thị trường → NCC (vào)", marketToFactoryIn, `Quy đổi sang ${factoryCurrency} theo tỷ giá hiện tại`);
...
const exportInventoryReport = async ({ products, openingStock, stockMovements, stockImportBatches, markets, warehouseFilter, dateFrom, dateTo, settings, exportedBy }) => {
  ...
  const movementsInRange = (stockMovements || []).filter(m =>
    m.status !== "cancelled" && inRange(m.date) &&
  ...
  const batchesInRange = (stockImportBatches || []).filter(b =>
...
const exportMarketReport = async ({ marketName, isAllMarkets, pos, shipments, payments, factories, products, markets, warranties = [], dateFrom, dateTo, settings, exportedBy }) => {
  const inRange = (d) => {
    if (!d) return true;
    if (dateFrom && d < dateFrom) return false;
    if (dateTo && d > dateTo) return false;
  ...
  let grandShippedCNY = 0, grandReceivedCNY = 0, grandPaidCNY = 0;
  ...
  targetMarkets.forEach(mName => {
    ...
    paidCNY = allTargetPayments.filter(p => p.fromMarket === mName).reduce((sum, p) => {
      const vnd = toVND(Number(p.amount || 0), p.currency, settings);
      return sum + vnd / (settings.cnyToVnd || 1);
    }, 0);
    marketSummaries[mName] = { shippedCNY, receivedCNY, paidCNY };
  ...
  pushMoneyRow("(3) Đã thanh toán cho nhà máy", grandPaidCNY, "Tổng các giao dịch Thị trường → NCC, quy đổi CNY");
  ...
  // v18: Còn phải trả = (1) − (3) − (3.5 hàng treo BH)
  const stillOwed = Math.max(0, grandShippedCNY - grandPaidCNY - grandWarrantyCNY);
  const creditFund = Math.max(0, grandPaidCNY + grandWarrantyCNY - grandShippedCNY);
  pushMoneyRow("(4) CÒN PHẢI TRẢ", stillOwed, "= (1) − (3) − (3.5), tối thiểu 0", true);
  pushMoneyRow("(5) QUỸ TÍN DỤNG THỊ TRƯỜNG", creditFund, "Nếu đã trả dư + treo BH so với hàng đã ship", true);
  ...
  const s3Rows = [];
  s3Rows.push([{ value: `LỊCH SỬ THANH TOÁN CHO NCC — ${titleSuffix}`, style: "sTitle", mergeAcross: 9 }]);
  s3Rows.push([]);
  const s3Headers = (isAllMarkets ? ["Thị trường"] : []).concat([
    "Mã TT", "Ngày", "NCC nhận", "Số tiền", "Tiền tệ", "Tỷ giá → VND",
    "Quy đổi CNY", "Quy đổi VND", "Người TT", "Ghi chú"
  ]);
  s3Rows.push(s3Headers.map(h => ({ value: h, style: "sHeader" })));
  ...
  if (allTargetPayments.length === 0) {
    ...
    s3Rows.push([]);
    const totalLabelMerge = isAllMarkets ? 7 : 6;
    s3Rows.push([
  ...
  const s3ColWidths = (isAllMarkets ? [85] : []).concat([110, 80, 170, 100, 60, 95, 110, 130, 110, 200]);
...
const POForm = ({ initial, factories, products, onSave, onClose }) => {
  ...
  const addItem = () => {
    ...
    // v13: Bỏ field produced
    setForm(p => ({ ...p, items: [...p.items, { id: `it${uid()}`, productId: prod.id, quantity: 0, unitPrice: prod.unitPrice }] }));
  };
        ...
        {form.items.map((it, idx) => {
          ...
          return (
            <div key={idx} style={{ display: "grid", gridTemplateColumns: "1fr 110px 120px 40px", gap: 8, marginBottom: 8, padding: 10, background: C.white, borderRadius: 10, alignItems: "center" }}>
              ...
              <input type="number" step="0.01" min={0} value={it.unitPrice} onChange={e => updateItem(idx, "unitPrice", e.target.value)} placeholder="Đơn giá" />
                  ...
                  {isExpanded && (
                    <tr>
                      <td colSpan={11} style={{ background: C.green50, padding: 24 }}>
                        ...
                        <table>
                          <thead><tr><th>SKU</th><th>Sản phẩm</th><th>Đơn giá</th><th>SL đặt</th><th>Đã giao</th><th>Còn lại</th><th>Giá trị</th></tr></thead>
                          <tbody>
                            {items.map(it => {
                              const prod = products.find(x => x.id === it.productId);
                              const itemShipped = p.items ? shippedFromItem(p.id, it.id, shipments) : shippedFromPO(p.id, shipments);
                              const itemRemain = Math.max(0, Number(it.quantity) - itemShipped);
...
const ShipmentForm = ({ initial, pos, shipments: allShipments, factories, products, markets, carriers, settings, onSave, onCreateWarehouse, onClose }) => {
  ...
  const [form, setForm] = useState(initial ? { ...initial, warehouseId: defaultWhId, documents: initial.documents || [] } : {
    id: "",
  ...
  const availableLines = useMemo(() => {
    ...
    pos.forEach(po => {
      ...
      const poItems = getPOItems(po);
      poItems.forEach(it => {
        // v26: Loại trừ cả Hủy + Nháp khỏi alreadyShipped
        const alreadyShipped = allShipments.filter(s => s.id !== initial?.id && isOperationalShipment(s))
          .flatMap(s => s.items || [])
    ...
    <Modal
      ...
      extraButton={form.status === "Nháp" && (
        <button
          ...
          onClick={() => onSave({ ...form, status: "Chờ xuất" })}
      ...
      width={900}>
      <div className="form-grid">
        <div className="form-group" style={{ gridColumn: "1/-1" }}><label>Mã đơn giao hàng {initial ? "" : "(tùy chọn)"}</label>
        ...
        {form.fees.map((fee, idx) => (
          <div key={idx} style={{ padding: 10, background: C.white, borderRadius: 10, marginBottom: 8 }}>
            <div style={{ display: "grid", gridTemplateColumns: "160px 110px 70px 1fr 40px", gap: 6, marginBottom: 6 }}>
              ...
              <input type="number" min={0} value={fee.amount} onChange={e => updateFee(idx, "amount", e.target.value)} placeholder="Số tiền" step="0.01" />
...
const WarrantyForm = ({ initial, factories, markets, products, settings, onSave, onClose }) => {
  ...
  const addItem = () => {
    const available = products.filter(p => !form.items.some(it => it.productId === p.id));
    if (available.length === 0) return;
    setForm(p => ({ ...p, items: [...p.items, { productId: available[0].id, quantity: 1 }] }));
  };
  const updateItem = (idx, field, val) => set("items", form.items.map((it, i) => i === idx ? { ...it, [field]: val } : it));
  const removeItem = (idx) => set("items", form.items.filter((_, i) => i !== idx));
  ...
  const isValid = form.marketFrom && form.factoryId && form.sendDate && form.items.length > 0 && form.items.every(it => it.productId && Number(it.quantity) > 0);
        ...
        {form.items.map((it, idx) => {
          ...
          return (
            <div key={idx} style={{ display: "grid", gridTemplateColumns: "2fr 100px 140px 40px", gap: 8, marginBottom: 8, padding: 10, background: C.white, borderRadius: 10, alignItems: "center" }}>
              ...
              <input type="number" min="1" value={it.quantity} onChange={e => updateItem(idx, "quantity", e.target.value)} placeholder="SL" />
...
// ============================================================
const Inventory = ({ products, openingStock = [], stockMovements = [], stockImportBatches = [], markets = [], settings, onImportSave, onCancelBatch, user }) => {
...
// ============================================================
const Configuration = ({ data, user, onAdd, onEdit, onDelete, onSaveSettings, onCreateWarehouse }) => {
  ...
  return (
    <div>
      ...
      {activeSubtab === "factories" && <Factories factories={data.factories} pos={data.pos} shipments={data.shipments} settings={data.settings} onAdd={onAdd} onEdit={onEdit} onDelete={onDelete} user={user} />}
      {activeSubtab === "carriers" && <Carriers carriers={data.carriers} shipments={data.shipments} feePayments={data.feePayments} settings={data.settings} onAdd={onAdd} onEdit={onEdit} onDelete={onDelete} user={user} />}
...
// ============================================================
const MarketDebts = ({ pos, shipments, payments, factories, markets, settings, products, warranties = [], user }) => {
  const marketNames = getMarketNames(markets);
  const [expanded, setExpanded] = useState(null);
  ...
  const handleExport = async ({ marketName, isAllMarkets, dateFrom, dateTo }) => {
    if (!isAllMarkets && !marketName) { alert("Vui lòng chọn thị trường"); return; }
    setExporting(true);
    try {
  ...
  const balances = useMemo(() => {
    const res = {};
    marketNames.forEach(m => { res[m] = calcMarketBalance(m, pos, shipments, payments, settings, warranties, products); });
    return res;
  }, [pos, shipments, payments, settings, warranties, products]);

  const summary = useMemo(() => {
    let totalReceived = 0, totalPaid = 0, totalOwed = 0, totalCredit = 0, totalWarrantyPending = 0;
    ...
    return { totalReceived, totalPaid, totalOwed, totalCredit, totalWarrantyPending };
  }, [balances]);
  ...
  return (
    <div>
      ...
      <div className="alert alert-info" style={{ marginBottom: 16 }}>
        <b>Cách tính Còn phải trả:</b> Hàng đã ship − Đã thanh toán − Hàng đang BH (treo). Hàng bảo hành ở NM (Đang gửi NM / NM đang sửa / Đang trả về kho) tạm trừ khỏi công nợ. Khi hàng "Đã trả về kho" hoặc "Hủy" → quay về công nợ bình thường.
      </div>

      {/* v18: KPI 5 thẻ — thêm "Hàng đang BH" */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 12, marginBottom: 20 }}>
        {[
          { label: "Hàng đã ship", val: summary.totalReceived, color: C.green700, sub: "Lũy kế đến hiện tại" },
          { label: "Đã thanh toán", val: summary.totalPaid, color: C.blue, sub: "TT → NM lũy kế" },
          { label: "Hàng đang BH", val: summary.totalWarrantyPending, color: C.orange, sub: "Đang treo công nợ" },
                ...
                <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                  ...
                  {b.warrantyPending > 0 && (
                    <div style={{ textAlign: "right" }}>
                      ...
                      <div style={{ fontWeight: 700, color: C.orange }}>{fmt(b.warrantyPending, "CNY")}</div>
...
const Payments = ({ pos, shipments, payments, factories, openingBalances, markets, settings, onAdd, onEdit, onDelete, user }) => {
  const marketNames = getMarketNames(markets);
  const [modal, setModal] = useState(null);
  const [filter, setFilter] = useState({ type: "", factory: "", showCancelled: true });
  const [dateFrom, setDateFrom] = useState("");
  const [dateTo, setDateTo] = useState("");
  ...
  // v28: Pagination — reverse trước khi paginate (mới nhất lên đầu)
  const filteredReversed = useMemo(() => filtered.slice().reverse(), [filtered]);
  const { page, setPage, pageSize, setPageSize, paginatedItems: pagedFiltered } = usePagination(filteredReversed, 50);
  ...
  return (
    <div>
      ...
      <div className="card" style={{ padding: 0, overflow: "hidden" }}>
        <table>
          <thead><tr><th>Mã</th><th>Loại</th><th>Nguồn</th><th>Đích</th><th>Số tiền</th><th>Ngày</th><th>Ghi chú</th>{(canEdit || canCancel) && <th></th>}</tr></thead>
          <tbody>
            {pagedFiltered.map(p => {
...
const PaymentEditForm = ({ initial, factories, markets, settings, isAdmin, onSave, onClose }) => {
  ...
  // - Sửa giới hạn (mọi user): amount, payDate, currency
  ...
  return (
    <Modal title={`Sửa thanh toán ${initial.id}`} onClose={onClose} onSave={() => onSave(form)} width={680}>
      ...
      <div className="form-grid">
        ...
        <div className="form-group"><label>Tiền tệ</label>
          <select value={form.currency || "CNY"} onChange={e => set("currency", e.target.value)}>
...
const AuditLog = ({ auditLog }) => {
  ...
  const filtered = useMemo(() => {
    ...
    if (actionFilter) {
      ...
      else if (actionFilter === "delete") items = items.filter(x => x.action.startsWith("delete"));
  ...
  return (
    <div>
      ...
      <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 16, padding: "14px 18px", marginBottom: 16, display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <select value={actionFilter} onChange={e => setActionFilter(e.target.value)} style={{ width: 170 }}>
...
const OpeningBalances = ({ openingBalances, factories, settings, onAdd, onEdit, onDelete, user }) => {
  ...
  return (
    <div>
      ...
      <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 16, padding: "14px 18px", marginBottom: 16, display: "flex", gap: 16, flexWrap: "wrap", alignItems: "center" }}>
            ...
            {filtered.length === 0 ? (
              ...
              return (
                <tr key={o.id} style={isCancelled ? { background: C.bg } : {}}>
                  ...
                  <td style={rowStyle}><Badge label={o.type === "debt" ? "Nợ gốc" : "Quỹ tín dụng đầu kỳ"} color={o.type === "debt" ? C.red : C.green600} /></td>
      ...
      {modal?.type === "new" && <OpeningBalanceForm factories={factories} onSave={f => { onAdd("openingBalances", { id: `OB-${uid()}`, status: "active", ...f }); setModal(null); }} onClose={() => setModal(null)} />}
      {modal?.type === "edit" && <OpeningBalanceForm initial={modal.data} factories={factories} onSave={f => { onEdit("openingBalances", modal.data.id, { ...f, lastEditedBy: user?.fullName || user?.username, lastEditedAt: new Date().toISOString() }); setModal(null); }} onClose={() => setModal(null)} />}

      {/* v20: Dialog hủy opening balance */}
      {cancelDlg && <PromptDialog title={`Hủy công nợ đầu kỳ ${cancelDlg.ob.id}?`}
        message="Sau khi hủy, công nợ đầu kỳ này sẽ KHÔNG tính vào số dư đầu kỳ của NCC nhưng vẫn lưu để audit."
        placeholder="VD: Sai số tiền, không đúng NCC, đã đối soát lại..."
        confirmLabel="🚫 Xác nhận Hủy" required={true}
        onConfirm={(reason) => {
...
export default function App() {
  ...
  // v33: ref để startAutoSync luôn đọc được data mới nhất mà không cần re-subscribe
  const dataRef = useRef(data);
  useEffect(() => { dataRef.current = data; }, [data]);

  useEffect(() => {
    (async () => {
      // v33: Load data từ S3 (qua s3Storage.loadAll — S3 → localStorage cache → memory)
      // Nếu S3 chưa có data → thử migrate từ legacy localStorage keys (v23…v9)
      let saved = await loadAll();
      let migratedFrom = "";
      if (!saved) {
        const LEGACY_VERSIONS = ["v23", "v22", "v21", "v20", "v19", "v18", "v17", "v16", "v15", "v14", "v13", "v12", "v11", "v10", "v9"];
        ...
        if (migratedFrom) console.info(`[GoChek CRM] Migrating data from ${migratedFrom} → S3 (crm_data_v12)`);
      }
      ...
      if (saved) {
        ...
        setData(d => ({ ...d, ...saved }));
        // v33: Luôn save lại để fix data cũ + sync lên S3
        await saveAll(saved);
      } else {
  ...
  // v33: Auto sync — start sau khi data loaded, cleanup khi unmount
  useEffect(() => {
    if (!loaded) return;
    startAutoSync(
      () => dataRef.current,
  ...
  // v33: save dùng saveAll từ s3Storage (debounced S3 PUT + localStorage cache)
  // Auto rebuild AUTO movements từ shipments/warranties trước khi persist
  ...
  const onAdd = async (key, item) => {
    ...
    // Rebuild stock movements sau khi addItem (nếu liên quan đến shipments/warranties)
    const synced = { ...next, stockMovements: rebuildAutoMovements(next.shipments || [], next.warranties || [], next.stockMovements || []) };
    setData(synced);
  };
  ...
  const onDelete = (key, id) => {
    ...
    setConfirmDialog({
      ...
      onConfirm: async () => {
        ...
        // v33: Hard delete vẫn dùng save() trực tiếp (filter ra khỏi array) vì s3Storage.softDeleteItem
        // dùng cơ chế soft-delete (deleted: true) — UI cần đồng nhất cách hiển thị trước khi chuyển.
  ...
  const onSaveSettings = async (newSettings) => {
    ...
    const next = await s3SaveSettings(data, newSettings, newLog);
    setData(next);
  };
  ...
  // v33: onCreateWarehouse dùng s3SaveMarkets từ s3Storage
  const onCreateWarehouse = async (marketName, newWh) => {
    ...
    const next = await s3SaveMarkets(data, updatedMarkets, newLog);
    setData(next);
  };
  ...
  const handleLogout = () => {
    if (user) {
      ...
      const nextData = { ...data, auditLog: [...data.auditLog, log] };
      // v33: Flush ngay lên S3 khi logout (không đợi debounce)
      s3Flush(nextData);
      setData(nextData);
    }
    setUser(null);
      ...
      <div style={{ display: "flex", minHeight: "100vh", background: C.bg }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
          ...
          <div style={{ padding: 28, flex: 1 }}>
            ...
            {tab === "products" && <Products products={data.products} pos={data.pos} shipments={data.shipments} factories={data.factories} markets={data.markets} settings={data.settings} onAdd={onAdd} onEdit={onEdit} onDelete={onDelete} onSaveSettings={onSaveSettings} user={user} />}
            ...
            {tab === "market_debts" && <MarketDebts pos={data.pos} shipments={data.shipments} payments={data.payments} factories={data.factories} markets={data.markets} settings={data.settings} products={data.products} warranties={data.warranties} user={user} />}
            ...
            {tab === "configuration" && <Configuration data={data} user={user} onAdd={onAdd} onEdit={onEdit} onDelete={onDelete} onSaveSettings={onSaveSettings} onCreateWarehouse={onCreateWarehouse} />}
