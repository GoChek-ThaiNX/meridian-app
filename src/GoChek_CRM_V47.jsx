...
// ============================================================
// GoChek Factory CRM — Phiên bản V38o
// ============================================================
// CHANGELOG V38o (vs V38n) — 2 vấn đề:
//
// VẤN ĐỀ 1 — Thêm trạng thái shipment "Đang xếp tàu":
//   - Vị trí: ngay TRƯỚC "Đang thông quan" trong SHIPMENT_STATUSES
//   - Tác động công nợ: tương đương "Đang thông quan" → đã rời NM → phát sinh công nợ NCC
//   - Cập nhật 4 mảng phái sinh: SHIPMENT_STATUS_ORDER, SHIPMENT_LEFT_FACTORY,
//     SHIPMENT_IN_TRANSIT, SHIPMENT_IN_TRANSIT_STATUSES
//   - Cập nhật map màu + icon (Dashboard, badges)
//
// VẤN ĐỀ 2 — Multi-select filter trạng thái (4 tab chính):
//   - Tạo component MultiSelectStatus mới (dropdown checkbox)
//   - Áp dụng cho: PO (Đặt hàng), Shipment (Giao hàng), Warranty (Bảo hành),
//     Payment stage (Thanh toán)
//   - Đổi state từ string → array. Logic filter dùng .includes()
//   - Tương thích ngược: state cũ là string sẽ tự convert sang array
//
// LƯU Ý:
//   - Các filter khác (Audit log action, Report PO deliveryStatus, Inventory alert,
//     Payment type) giữ single-select để không phình app
//   - "Đang xếp tàu" có thể KHÔNG có data cũ → không cần migration
//
// ============================================================
// GoChek Factory CRM — Phiên bản V38n
// ============================================================
// CHANGELOG V38n (vs V38m) — 4 vấn đề:
//
// VẤN ĐỀ 1 — Tab Giao hàng: bổ sung select NCC (lọc shipment theo NCC).
// VẤN ĐỀ 2 — Tab Giao hàng: Tổng giá trị dùng fmt() đầy đủ (¥ + ≈ VNĐ đầy đủ),
//            thay vì fmtVND rút gọn K/M/B.
// VẤN ĐỀ 3 — Tab Thanh toán: bổ sung select thị trường (filter fromMarket).
//            Chỉ áp dụng cho payment TT → NM. INTER_FACTORY bị ẩn khi lọc.
// VẤN ĐỀ 4 — Tab Công nợ TT: lọc kỳ theo CÁCH A.
//            Khi có filter ngày: hiện PS nợ + PS trả + Chênh lệch trong kỳ.
//            Bảo hành ship trong kỳ → tính làm treo nợ trong kỳ.
//            Khi không filter: giữ nguyên view tích lũy như cũ.
//            Excel đồng bộ Cách A.
//
// LOGIC PS trong kỳ:
//   - PS nợ = Σ giá trị shipment có departDate trong [dateFrom, dateTo], loại Hủy + Nháp
//   - PS trả = Σ payment hoàn tất có payDate trong [dateFrom, dateTo], loại hủy
//   - Đang treo trong kỳ = Σ payment stage 1+2 có payDate trong [dateFrom, dateTo]
//   - BH treo nợ trong kỳ = Σ warranty có sendDate trong [dateFrom, dateTo], pending
//   - Chênh lệch = PS nợ − PS trả
//
// HELPER MỚI:
//   - calcMarketBalanceInPeriod(market, dateFrom, dateTo, ...) — chỉ tính PS trong kỳ
//   - calcMarketBalance cũ giữ nguyên (không động chạm) → Dashboard, các nơi khác không bị ảnh hưởng
//
// ============================================================
// GoChek Factory CRM — Phiên bản V38m
// ============================================================
// CHANGELOG V38m (vs V38L) — 7 mục:
//
// MỤC TIÊU: Báo cáo tiến độ PO — theo dõi tổng thể giao hàng + thanh toán.
//
// CẤU TRÚC 3 CẤP:
//   - Cấp NCC (nhóm): số thanh toán THẬT từ calcFactoryBalance
//   - Cấp PO (con của NCC): giá trị giao hàng theo từng PO
//   - Cấp SP (expand): đặt/giao/còn từng sản phẩm trong PO
//
// LOGIC CỐT LÕI:
//   - Công nợ chỉ phát sinh khi HÀNG ĐÃ GIAO (không phải khi đặt PO)
//   - Tiền đang treo (uỷ thác + đang chuyển QT) TÁCH RIÊNG khỏi "Đã trả"
//   - Thanh toán chỉ hiện ở cấp NCC (số thật, không suy đoán FIFO)
//   - Giao hàng hiện 2 %: trên giá trị PO + trên giá trị đã giao
//
// 7 MỤC:
// 1. Sub-tab "📊 Báo cáo tiến độ" trong tab Đơn đặt hàng
// 2. Component POProgressReport — nhóm NCC → PO → SP
// 3. Logic tính: giá trị giao/PO theo SP, 3 chỉ số NCC, 2 % giao hàng
// 4. Thanh tổng hợp 4 ô + cảnh báo PO treo lâu (>90 ngày)
// 5. Bộ lọc (NCC / trạng thái giao / thời gian)
// 6. Báo cáo Excel 3 sheets (Tổng hợp NCC / Chi tiết PO / Chi tiết PO × SP)
// 7. Help docs + version history + invariant test
//
// ============================================================
// GoChek Factory CRM — Phiên bản V38L
// ============================================================
// CHANGELOG V38L (vs V38k) — 6 mục:
//
// MỤC TIÊU: Nhật ký ghi tác động chi tiết đến công nợ NCC.
//
// Khi thực hiện thao tác trên đơn đặt hàng / giao hàng / bảo hành / thanh toán /
// công nợ đầu kỳ, nhật ký ghi thêm tác động lên 3 chỉ số công nợ NCC:
//   - Còn phải trả (stillOwed)
//   - Đã thanh toán (netPaid)
//   - Quỹ tín dụng (creditFund)
// Ghi dạng "X → Y" (trước → sau).
//
// 6 MỤC:
// 1. Helper snapshotFactoryDebt — chụp 3 chỉ số 1 NCC
// 2. Helper buildDebtImpactLog — so sánh trước/sau, build mô tả
// 3. Tích hợp vào handler: onAdd/onEdit (PO duyệt, shipment, warranty, payment, OB)
// 4. Giao diện AuditLog: mô tả rút gọn 50 ký tự + nút "xem thêm"
// 5. Block "Tác động công nợ NCC" khi mở rộng log
// 6. Verify + invariant test
//
// PHẠM VI:
//   - PO: chỉ ghi tác động khi DUYỆT (Chờ duyệt → Đã duyệt)
//   - Shipment / Warranty / Payment / OB: ghi cả tạo + sửa + hủy
//   - Bảo hành không trực tiếp đổi công nợ NCC → ghi "không ảnh hưởng" nếu 3 chỉ số không đổi
//   - Sửa record đổi NCC → ghi tác động lên cả 2 NCC
//
// ============================================================
...
const SHIPMENT_STATUSES = ["Nháp", "Chờ xuất", "Đang vận chuyển TQ", "Đang xếp tàu", "Đang thông quan", "Kiểm hoá", "Đã thông quan", "Đã về kho", "Hủy"];
      ...
      },
      {
        ...
        content: [
          { type: "table", headers: ["Phiên bản", "Tính năng chính"], rows: [
            ...
            ["V38L", "📒 Nhật ký ghi tác động công nợ NCC. Khi tạo/sửa/hủy đơn đặt hàng (lúc duyệt) / giao hàng / bảo hành / thanh toán / công nợ đầu kỳ → nhật ký ghi thêm 3 chỉ số công nợ NCC (Còn phải trả, Đã thanh toán, Quỹ tín dụng) thay đổi từ X → Y. Giao diện nhật ký rút gọn 50 ký tự + nút 'xem thêm' mở chi tiết."],
            ["V38m", "📊 Báo cáo tiến độ PO — sub-tab mới trong tab Đơn đặt hàng. Nhóm theo NCC → PO → SP (3 cấp expand). Thanh toán hiện ở CẤP NCC (số thật từ calcFactoryBalance, không suy đoán FIFO). Giao hàng hiện 2 thanh tiến độ. 5 trạng thái: Chờ duyệt / Chưa giao / Đang giao / Giao đủ / 🔴 Treo lâu (>90 ngày). Thanh tổng hợp 5 ô + cảnh báo PO treo lâu. Bộ lọc theo NCC/trạng thái/thời gian. Xuất Excel 3 sheets (Tổng hợp NCC / Chi tiết PO / Chi tiết PO×SP). Cũng thêm thanh Tổng hợp ở tab Thanh toán (số GD + Hoàn tất + Đang chuyển)."],
            ["V38n", "🔍 Bổ sung filter + lọc kỳ. (1) Tab Giao hàng: thêm select NCC + sửa Tổng giá trị dùng VNĐ đầy đủ. (2) Tab Thanh toán: thêm select thị trường (lọc fromMarket). (3) Tab Công nợ TT: lọc kỳ theo Cách A — khi chọn ngày sẽ hiện PS nợ + PS trả + BH treo + Chênh lệch trong kỳ; không filter giữ view tích lũy cũ. BH ship trong kỳ tính làm treo nợ. Excel TT đồng bộ Cách A."],
            ["V38o", "🚢 Thêm trạng thái shipment 'Đang xếp tàu' (đặt giữa Đang VC TQ và Đang thông quan, tác động công nợ như Đang thông quan). 🏷️ Multi-select filter trạng thái cho 4 tab chính (PO/Shipment/Warranty/Payment) — component MultiSelectStatus dạng dropdown checkbox, hỗ trợ chọn nhiều trạng thái cùng lúc."],
          ]},
        ],
      },
    ],
...
const SHIPMENT_STATUS_ORDER = ["Nháp", "Chờ xuất", "Đang vận chuyển TQ", "Đang xếp tàu", "Đang thông quan", "Kiểm hoá", "Đã thông quan", "Đã về kho"];
...
// ============================================================
// STORAGE & UTILS
// v47: Storage được chuyển sang s3Storage.js (S3 + localStorage cache + memory fallback)
...
// ============================================================
// v38L: DEBT IMPACT HELPERS — Ghi tác động công nợ NCC vào nhật ký
// ============================================================

// Chụp 3 chỉ số công nợ của 1 NCC tại thời điểm hiện tại.
// Trả về { stillOwed, netPaid, creditFund } — đơn vị CNY.
const snapshotFactoryDebt = (factoryId, data, settings) => {
  if (!factoryId) return null;
  const bal = calcFactoryBalance(
    factoryId,
    data.pos || [],
    data.shipments || [],
    data.payments || [],
  ...
  return {
    stillOwed: bal.stillOwed || 0,
    netPaid: bal.netPaid || 0,
    creditFund: bal.creditFund || 0,
  };
};

// So sánh 2 snapshot, build mô tả tác động công nợ.
// Trả về { factoryId, factoryName, changed, lines: [{ label, before, after, delta }] }
const buildDebtImpactLog = (factoryId, before, after, factories) => {
  if (!factoryId || !before || !after) return null;
  const factory = (factories || []).find(f => f.id === factoryId);
  const factoryName = factory?.name || factoryId;
  const metrics = [
    { key: "stillOwed", label: "Còn phải trả" },
    { key: "netPaid", label: "Đã thanh toán" },
    { key: "creditFund", label: "Quỹ tín dụng" },
  ];
  const lines = metrics.map(m => {
    const b = Math.round(before[m.key] || 0);
    const a = Math.round(after[m.key] || 0);
    return { label: m.label, before: b, after: a, delta: a - b };
  });
  const changed = lines.some(l => l.delta !== 0);
  return { factoryId, factoryName, changed, lines };
};

// Lấy danh sách factoryId liên quan đến 1 record (theo loại entity).
// key: "pos" | "shipments" | "payments" | "warranties" | "openingBalances"
// record: object record (có thể là bản trước hoặc sau khi sửa)
// data: store hiện tại (cần để map shipment item → PO → factory)
const getRelatedFactoryIds = (key, record, data) => {
  if (!record) return [];
  const ids = new Set();
  if (key === "pos") {
    if (record.factoryId) ids.add(record.factoryId);
  } else if (key === "warranties") {
    if (record.factoryId) ids.add(record.factoryId);
  } else if (key === "openingBalances") {
    if (record.factoryId) ids.add(record.factoryId);
  } else if (key === "payments") {
    if (record.toFactoryId) ids.add(record.toFactoryId);
    if (record.fromFactoryId) ids.add(record.fromFactoryId);
  } else if (key === "shipments") {
    // Map mỗi item.poId → PO → factoryId
    (record.items || []).forEach(it => {
      const po = (data.pos || []).find(p => p.id === it.poId);
      if (po?.factoryId) ids.add(po.factoryId);
    });
  }
...
const SHIPMENT_LEFT_FACTORY = ["Chờ xuất", "Đang vận chuyển TQ", "Đang vận chuyển", "Đang xếp tàu", "Đang thông quan", "Kiểm hoá", "Đã thông quan", "Đã về kho"];
// v33: Các trạng thái shipment đang trên đường (đã rời NM, chưa về kho TT)
const SHIPMENT_IN_TRANSIT = ["Chờ xuất", "Đang vận chuyển TQ", "Đang vận chuyển", "Đang xếp tàu", "Đang thông quan", "Kiểm hoá", "Đã thông quan"];
...
// ============================================================
// v38n: calcMarketBalanceInPeriod — Tính PHÁT SINH TRONG KỲ (Cách A)
// ============================================================
// Chỉ tính các giao dịch xảy ra trong [dateFrom, dateTo]. KHÔNG có Đầu/Cuối kỳ.
// Trả về: psNoCNY (PS nợ), psTraCNY (PS trả), psPendingCNY (đang treo trong kỳ),
//         psWarrantyCNY (BH ship trong kỳ làm treo nợ), chenhLechCNY (PS nợ − PS trả − BH treo),
//         shipmentCount, paymentCount, warrantyCount.
// Logic:
//   - PS nợ = Σ shipment có departDate trong kỳ (loại Hủy + Nháp)
//   - PS trả = Σ payment hoàn tất (stage completed) có payDate trong kỳ (loại cancelled)
//   - PS đang treo = Σ payment stage 1+2 có payDate trong kỳ (loại cancelled)
//   - PS BH = Σ warranty pending có sendDate trong kỳ
//   - Chênh lệch = PS nợ + BH treo − PS trả (số > 0 = nợ tăng trong kỳ)
const calcMarketBalanceInPeriod = (market, dateFrom, dateTo, pos, shipments, payments, warranties = [], products = [], settings) => {
  const inRange = (d) => {
    if (!d) return false; // record không có ngày → không tính trong kỳ
    if (dateFrom && d < dateFrom) return false;
    if (dateTo && d > dateTo) return false;
    return true;
  };

  // PS nợ: Σ giá trị shipment trong kỳ
  let psNoCNY = 0;
  let shipmentCount = 0;
  shipments.filter(s =>
    s.market === market &&
    isOperationalShipment(s) &&
    inRange(s.departDate)
  ).forEach(s => {
    shipmentCount++;
    (s.items || []).forEach(i => {
      const po = pos.find(p => p.id === i.poId);
      ...
      const poItem = po.items ? poItems.find(x => x.id === i.itemId) : poItems[0];
      if (!poItem) return;
      const valInPoCurrency = Number(i.quantity || 0) * Number(poItem.unitPrice || 0);
      const valVND = toVND(valInPoCurrency, po.currency, settings);
      psNoCNY += valVND / (settings.cnyToVnd || 1);
    });
  });

  // PS trả + PS đang treo: payment trong kỳ
  let psTraCNY = 0;
  let psPendingCNY = 0;
  let paymentCount = 0;
  payments.filter(p =>
    p.type === "MARKET_TO_FACTORY" &&
    p.fromMarket === market &&
    p.status !== "cancelled" &&
    inRange(p.payDate)
  ).forEach(p => {
    ...
    const stage = getPaymentStage(p);
    if (stage === "completed") psTraCNY += cny;
    else if (PAYMENT_PENDING_STAGES.includes(stage)) psPendingCNY += cny;
  });

  // PS BH treo nợ: warranty ship trong kỳ + pending
  let psWarrantyCNY = 0;
  let warrantyCount = 0;
  (warranties || []).filter(w =>
    w.marketFrom === market &&
    WARRANTY_PENDING_STATUSES.includes(w.status) &&
    inRange(w.sendDate)
  ).forEach(w => {
    warrantyCount++;
    (w.items || []).forEach(it => {
      ...
      const valInProdCurrency = Number(it.quantity || 0) * Number(prod.unitPrice || 0);
      const valVND = toVND(valInProdCurrency, prod.currency || "CNY", settings);
      psWarrantyCNY += valVND / (settings.cnyToVnd || 1);
    });
  });

  // Chênh lệch = PS nợ + BH treo − PS trả (số > 0 = nợ tăng trong kỳ)
  const chenhLechCNY = psNoCNY + psWarrantyCNY - psTraCNY;
...
const SHIPMENT_IN_TRANSIT_STATUSES = ["Chờ xuất", "Đang vận chuyển TQ", "Đang xếp tàu", "Đang thông quan", "Kiểm hoá", "Đã thông quan", "Đang vận chuyển"];
...
// ============================================================
// v38m: exportPOProgressReport — Báo cáo tiến độ PO 3 sheets
// ============================================================
// Sheet 1: Tổng hợp NCC (mỗi dòng = 1 NCC, kèm thanh toán)
// Sheet 2: Chi tiết PO (mỗi dòng = 1 PO + tiến độ giao)
// Sheet 3: Chi tiết PO × SP (mỗi dòng = 1 SP trong PO)
const exportPOProgressReport = async ({ factoryRows, summary, shipments, products, filterFactoryName, dateFrom, dateTo, settings, exportedBy }) => {
  const today = new Date().toISOString().slice(0, 10);
  const cnyToVnd = settings?.cnyToVnd || 1;
  const dateRange = dateFrom || dateTo ? `${dateFrom || "—"} → ${dateTo || "—"}` : "Tất cả thời gian";

  // === Sheet 1: Tổng hợp NCC ===
  const s1Rows = [];
  s1Rows.push([{ value: "BÁO CÁO TIẾN ĐỘ PO — TỔNG HỢP NCC (V38m)", style: "sTitle", mergeAcross: 8 }]);
  s1Rows.push([{ value: `Xuất ngày: ${today}${exportedBy ? ` · Bởi: ${exportedBy}` : ""} · Kỳ: ${dateRange}${filterFactoryName ? ` · Lọc NCC: ${filterFactoryName}` : ""}`, style: "sCell", mergeAcross: 8 }]);
  s1Rows.push([]);
  s1Rows.push([
    { value: "🏭 NCC", style: "sHeader" },
    { value: "Số PO", style: "sHeader" },
    { value: "Tổng giá trị PO (¥)", style: "sHeader" },
    { value: "Đã giao (¥)", style: "sHeader" },
    { value: "% Đã giao", style: "sHeader" },
    { value: "Đã thanh toán (¥)", style: "sHeader" },
    { value: "Đang treo (¥)", style: "sHeader" },
    { value: "Còn phải trả (¥)", style: "sHeader" },
    { value: "Tổng (VNĐ)", style: "sHeader" },
  ]);
  if (factoryRows.length === 0) {
    s1Rows.push([{ value: "(Không có NCC nào)", style: "sCell", mergeAcross: 8 }]);
  } else {
    factoryRows.forEach(fr => {
      const pct = fr.totalPOCNY > 0 ? ((fr.deliveredCNY / fr.totalPOCNY) * 100).toFixed(1) + "%" : "—";
      s1Rows.push([
        { value: fr.factoryName, style: "sLabel" },
        { value: fr.poCount, style: "sCellNum" },
        { value: Math.round(fr.totalPOCNY), style: "sCellNum" },
        { value: Math.round(fr.deliveredCNY), style: "sCellNum" },
        { value: pct, style: "sCell" },
        { value: Math.round(fr.netPaid), style: "sGreen" },
        { value: Math.round(fr.pendingCNY), style: "sCellNum" },
        { value: Math.round(fr.stillOwed), style: fr.stillOwed > 0 ? "sRed" : "sCellNum" },
        { value: Math.round((fr.totalPOCNY) * cnyToVnd), style: "sCellNum" },
      ]);
    });
    // Tổng cuối
    s1Rows.push([
      { value: "TỔNG", style: "sLabel" },
      { value: summary.poCount, style: "sLabel" },
      { value: Math.round(summary.totalPOCNY), style: "sLabel" },
      { value: Math.round(summary.deliveredCNY), style: "sLabel" },
      { value: summary.totalPOCNY > 0 ? ((summary.deliveredCNY / summary.totalPOCNY) * 100).toFixed(1) + "%" : "—", style: "sLabel" },
      { value: Math.round(summary.paidCNY), style: "sLabel" },
      { value: Math.round(summary.pendingCNY), style: "sLabel" },
      { value: Math.round(summary.totalPOCNY - summary.paidCNY - summary.pendingCNY), style: "sLabel" },
      { value: Math.round((summary.totalPOCNY) * cnyToVnd), style: "sLabel" },
    ]);
  }
  const sheet1 = xmlWorksheet("Tổng hợp NCC", s1Rows, [200, 60, 130, 130, 80, 130, 130, 130, 150]);

  // === Sheet 2: Chi tiết PO ===
  const s2Rows = [];
  s2Rows.push([{ value: "BÁO CÁO TIẾN ĐỘ PO — CHI TIẾT TỪNG PO", style: "sTitle", mergeAcross: 9 }]);
  s2Rows.push([{ value: `Tổng ${summary.poCount} PO`, style: "sCell", mergeAcross: 9 }]);
  s2Rows.push([]);
  s2Rows.push([
    { value: "🏭 NCC", style: "sHeader" },
    { value: "Mã PO", style: "sHeader" },
    { value: "Ngày đặt", style: "sHeader" },
    { value: "Số SP", style: "sHeader" },
    { value: "Giá trị PO (¥)", style: "sHeader" },
    { value: "Đã giao (¥)", style: "sHeader" },
    { value: "% Đã giao", style: "sHeader" },
    { value: "Còn lại (¥)", style: "sHeader" },
    { value: "Trạng thái giao", style: "sHeader" },
    { value: "Tiền tệ", style: "sHeader" },
  ]);
  factoryRows.forEach(fr => {
    fr.pos.forEach(po => {
      const poTotal = calcPOTotalValueCNY(po);
      const poDelivered = calcPODeliveredValueCNY(po, shipments);
      const poRemain = poTotal - poDelivered;
      const pct = poTotal > 0 ? ((poDelivered / poTotal) * 100).toFixed(1) + "%" : "—";
      const st = getPODeliveryStatus(po, shipments);
      const itemCount = (getPOItems(po) || []).length;
      const styleStatus = st.id === "overdue" ? "sRed" : st.id === "delivered" ? "sGreen" : "sCell";
      s2Rows.push([
        { value: fr.factoryName, style: "sCell" },
        { value: po.id, style: "sLabel" },
        { value: po.orderDate || "—", style: "sCell" },
        { value: itemCount, style: "sCellNum" },
        { value: Math.round(poTotal), style: "sCellNum" },
        { value: Math.round(poDelivered), style: "sCellNum" },
        { value: pct, style: "sCell" },
        { value: Math.round(poRemain), style: poRemain > 0 ? "sCellNum" : "sGreen" },
        { value: `${st.icon} ${st.label}`, style: styleStatus },
        { value: po.currency || "CNY", style: "sCell" },
      ]);
    });
  });
  const sheet2 = xmlWorksheet("Chi tiết PO", s2Rows, [180, 110, 90, 60, 120, 120, 80, 120, 120, 70]);

  // === Sheet 3: Chi tiết PO × SP ===
  const s3Rows = [];
  s3Rows.push([{ value: "BÁO CÁO TIẾN ĐỘ PO — CHI TIẾT TỪNG SP TRONG PO", style: "sTitle", mergeAcross: 9 }]);
  s3Rows.push([{ value: "Mỗi dòng = 1 SP trong 1 PO", style: "sCell", mergeAcross: 9 }]);
  s3Rows.push([]);
  s3Rows.push([
    { value: "🏭 NCC", style: "sHeader" },
    { value: "Mã PO", style: "sHeader" },
    ...
    { value: "SKU", style: "sHeader" },
    { value: "Tên SP", style: "sHeader" },
    { value: "SL đặt", style: "sHeader" },
    { value: "Đã giao", style: "sHeader" },
    { value: "Còn lại", style: "sHeader" },
    { value: "Đơn giá", style: "sHeader" },
    { value: "Giá trị đặt", style: "sHeader" },
    { value: "Giá trị giao", style: "sHeader" },
    { value: "Tiền tệ", style: "sHeader" },
  ]);
  factoryRows.forEach(fr => {
    fr.pos.forEach(po => {
      ...
      items.forEach(it => {
        const prod = products.find(p => p.id === it.productId);
        const shipped = po.items ? shippedFromItem(po.id, it.id, shipments) : shippedFromPO(po.id, shipments);
        const remain = Math.max(0, Number(it.quantity) - shipped);
        const itemTotal = Number(it.quantity) * Number(it.unitPrice || 0);
        const itemDelivered = shipped * Number(it.unitPrice || 0);
        s3Rows.push([
          { value: fr.factoryName, style: "sCell" },
          { value: po.id, style: "sLabel" },
          { value: po.orderDate || "—", style: "sCell" },
          { value: prod?.sku || "—", style: "sLabel" },
          { value: prod?.name || "—", style: "sCell" },
          { value: Number(it.quantity || 0), style: "sCellNum" },
          { value: shipped, style: "sCellNum" },
          { value: remain, style: remain > 0 ? "sCellNum" : "sGreen" },
          { value: Number(it.unitPrice || 0), style: "sCellNum" },
          { value: Math.round(itemTotal), style: "sCellNum" },
          { value: Math.round(itemDelivered), style: "sCellNum" },
          { value: po.currency || "CNY", style: "sCell" },
        ]);
      });
    });
  });
  const sheet3 = xmlWorksheet("Chi tiết PO x SP", s3Rows, [180, 110, 90, 90, 200, 80, 80, 80, 90, 110, 110, 70]);

  // === Build file ===
  const allSheets = [sheet1, sheet2, sheet3].join("\n");
  const filename = `BaoCao_TienDo_PO_${today}.xls`;
  downloadXlsFile(allSheets, filename);
  return filename;
};
...
const exportMarketReport = async ({ marketName, isAllMarkets, pos, shipments, payments, factories, products, markets, warranties = [], openingBalances = [], dateFrom, dateTo, settings, exportedBy }) => {
  ...
  s1Rows.push([{ value: "BẢNG CÔNG NỢ — PHÁT SINH TRONG KỲ (Cách A)", style: "sSection", mergeAcross: 3 }]);
  // v38n: Excel theo Cách A — chỉ PS trong kỳ. Không có Đầu/Cuối kỳ.
  // Cũ V38h: Còn phải trả = (1) + OB − (3) − (3a) − BH → trộn tích lũy + kỳ.
  // Mới V38n: Chênh lệch trong kỳ = PS nợ + BH treo trong kỳ − PS trả trong kỳ.
  s1Rows.push([
    { value: "KHOẢN MỤC", style: "sHeader" },
  ...
  pushMoneyRow("(1) + PS Nợ trong kỳ (hàng đã ship)", grandShippedCNY, `Σ shipment có ngày xuất trong kỳ (loại Hủy + Nháp)`);
  pushMoneyRow("(2) Tham chiếu — Hàng đã NHẬN về kho trong kỳ", grandReceivedCNY, "Để đối chiếu hao hụt với (1)");
  pushMoneyRow("(3) − PS Trả trong kỳ (✅ stage Hoàn tất)", grandPaidCNY, "Σ payment hoàn tất có ngày trả trong kỳ");
  pushMoneyRow("(3a) 🟡 Đang treo trong kỳ (stage 🏦 + 🌐)", grandPendingCNY, "Σ payment đang chuyển/uỷ thác có ngày trả trong kỳ");

  // v38n: BH treo trong kỳ — chỉ tính warranty có sendDate trong kỳ + đang pending
  let grandWarrantyCNY = 0;
  let warrantyCountInPeriod = 0;
  (warranties || []).filter(w =>
    targetMarkets.includes(w.marketFrom) &&
    WARRANTY_PENDING_STATUSES.includes(w.status) &&
    inRange(w.sendDate)
  ).forEach(w => {
    warrantyCountInPeriod++;
    (w.items || []).forEach(it => {
      const prod = products.find(p => p.id === it.productId);
      if (!prod) return;
      const valInProdCurrency = Number(it.quantity || 0) * Number(prod.unitPrice || 0);
      const valVND = toVND(valInProdCurrency, prod.currency || "CNY", settings);
      grandWarrantyCNY += valVND / (settings.cnyToVnd || 1);
    });
  });
  pushMoneyRow("(4) + BH treo trong kỳ", grandWarrantyCNY, `${warrantyCountInPeriod} lô BH ship trong kỳ — đang treo nợ`);

  s1Rows.push([]);

  // v38n: Chênh lệch = PS nợ + BH treo − PS trả. Số > 0 = nợ tăng trong kỳ.
  const chenhLech = grandShippedCNY + grandWarrantyCNY - grandPaidCNY;
  pushMoneyRow("(5) = CHÊNH LỆCH TRONG KỲ", chenhLech, chenhLech > 0 ? "Nợ TĂNG trong kỳ — TT chưa trả đủ phần phát sinh" : "Nợ GIẢM trong kỳ — TT đã trả vượt phần phát sinh", true);

  // Nếu xuất tất cả → thêm bảng break-down theo từng thị trường
  if (isAllMarkets) {
    s1Rows.push([]);
    s1Rows.push([{ value: "PHÂN TÁCH PS TRONG KỲ THEO TỪNG THỊ TRƯỜNG", style: "sSection", mergeAcross: 4 }]);
    s1Rows.push([
      { value: "Thị trường", style: "sHeader" },
      { value: "+ PS Nợ (CNY)", style: "sHeader" },
      { value: "− PS Trả (CNY)", style: "sHeader" },
      { value: "+ BH treo (CNY)", style: "sHeader" },
      { value: "= Chênh lệch (CNY)", style: "sHeader" },
    ]);
    targetMarkets.forEach(mName => {
      const ms = marketSummaries[mName];
      // BH treo trong kỳ riêng cho mName
      let warrantyCNY = 0;
      (warranties || []).filter(w =>
        w.marketFrom === mName &&
        WARRANTY_PENDING_STATUSES.includes(w.status) &&
        ...
        (w.items || []).forEach(it => {
          ...
          const valInProdCurrency = Number(it.quantity || 0) * Number(prod.unitPrice || 0);
          const valVND = toVND(valInProdCurrency, prod.currency || "CNY", settings);
          warrantyCNY += valVND / (settings.cnyToVnd || 1);
        });
      });
      const cl = ms.shippedCNY + warrantyCNY - ms.paidCNY;
      s1Rows.push([
        { value: mName, style: "sLabel" },
        { value: Math.round(ms.shippedCNY), style: "sCellNum" },
        { value: Math.round(ms.paidCNY), style: "sCellNum" },
        { value: Math.round(warrantyCNY), style: "sCellNum" },
        { value: Math.round(cl), style: cl > 0 ? "sRed" : "sGreen" },
      ]);
    });
  }
...
// ============================================================
// v38o: MultiSelectStatus — Dropdown checkbox để chọn nhiều trạng thái
// ============================================================
// Props:
//   options:        Array<string> — danh sách trạng thái
//   value:          Array<string> — các trạng thái đang chọn (rỗng = chọn tất cả)
//   onChange:       (newValue: Array<string>) => void
//   label:          string — placeholder khi chưa chọn (vd "Tất cả trạng thái")
//   width:          string|number (default 220)
//   renderColor:    (opt: string) => string — màu badge từng option (optional)
//
// UI: nút cố định hiển thị "🏷️ N trạng thái" hoặc "Tất cả", click mở dropdown checkbox.
// Có "Chọn tất cả" / "Bỏ chọn".
const MultiSelectStatus = ({ options = [], value = [], onChange, label = "Tất cả", width = 220, renderColor = null }) => {
  const [open, setOpen] = useState(false);
  const ref = useRef(null);

  // v38o: Đóng dropdown khi click ngoài
  useEffect(() => {
    if (!open) return;
  ...
  const allSelected = safeValue.length === 0 || safeValue.length === options.length;
  const displayLabel = allSelected ? `🏷️ ${label}` :
    safeValue.length === 1 ? `🏷️ ${safeValue[0]}` :
    `🏷️ ${safeValue.length} mục đã chọn`;
  ...
  const selectAll = () => onChange([]); // rỗng = tất cả
  const clearAll = () => onChange(["__NONE__"]); // marker không match gì
    ...
    <div ref={ref} style={{ position: "relative", width, display: "inline-block" }}>
      <button
        ...
        style={{
          ...
          alignItems: "center",
          color: safeValue.length === 0 ? C.textMuted : C.text,
          fontWeight: safeValue.length > 0 ? 600 : 400,
        }}
      >
        <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{displayLabel}</span>
        <span style={{ color: C.textMuted, marginLeft: 6 }}>{open ? "▲" : "▼"}</span>
      </button>
      {open && (
        <div style={{
          position: "absolute",
          ...
          <div style={{ display: "flex", gap: 4, padding: "8px 10px", borderBottom: `1px solid ${C.borderLight}`, background: C.bg }}>
            <button onClick={selectAll} className="btn btn-ghost" style={{ padding: "3px 8px", fontSize: 11 }}>Tất cả</button>
            <button onClick={clearAll} className="btn btn-ghost" style={{ padding: "3px 8px", fontSize: 11 }}>Bỏ chọn</button>
          </div>
          {options.map(opt => {
            const checked = safeValue.length === 0 ? false : safeValue.includes(opt);
            const color = renderColor ? renderColor(opt) : null;
            return (
              <label key={opt} style={{
                display: "flex",
                alignItems: "center",
                ...
                background: checked ? C.green50 : "transparent",
              }}>
                <input type="checkbox" checked={checked} onChange={() => toggle(opt)} style={{ accentColor: C.green500 }} />
                {color && <span style={{ display: "inline-block", width: 8, height: 8, borderRadius: 4, background: color }}></span>}
                <span style={{ color: C.text, fontWeight: checked ? 600 : 400 }}>{opt}</span>
              </label>
            );
          })}
        </div>
...
// Helper: kiểm tra value có match filter array hay không
// - filter là array rỗng → match tất cả
// - filter chứa "__NONE__" → không match gì
// - ngược lại → check .includes()
const matchStatusFilter = (filterArr, value) => {
  if (!Array.isArray(filterArr) || filterArr.length === 0) return true;
  if (filterArr.includes("__NONE__")) return false;
    ...
    <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: `linear-gradient(135deg, ${C.green50} 0%, ${C.green100} 100%)` }}>
      <div style={{ width: 420, background: C.white, borderRadius: 24, padding: 44, boxShadow: "0 20px 60px rgba(30, 60, 30, 0.15)" }}>
        <div style={{ textAlign: "center", marginBottom: 36 }}>
          <div style={{ width: 72, height: 72, borderRadius: 18, background: `linear-gradient(135deg, ${C.green400} 0%, ${C.green600} 100%)`, display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 18px", color: "white", fontSize: 30, fontWeight: 800, boxShadow: `0 8px 20px ${C.green500}40` }}>GC</div>
          <div style={{ fontSize: 26, color: C.green800, fontWeight: 800, letterSpacing: "-0.02em" }}>GC</div>
...
const Dashboard = ({ pos, shipments, payments, factories, products, openingBalances, markets, carriers, feePayments, stockOnHand = [], settings, onNavigate }) => {
  ...
  const stats = useMemo(() => {
    ...
    const inTransit = filteredShipments.filter(s => ["Đang vận chuyển TQ", "Đang vận chuyển", "Đang xếp tàu", "Đang thông quan", "Kiểm hoá", "Đã thông quan"].includes(s.status)).length;
...
// ============================================================
// v38m: POProgressReport — Báo cáo tiến độ PO
// ============================================================
// Nhóm theo NCC → PO → SP (3 cấp expand).
// Thanh toán hiện ở CẤP NCC (số thật từ calcFactoryBalance).
// Giao hàng hiện ở từng PO + từng SP.
// Cảnh báo PO "treo lâu" > 90 ngày chưa giao đủ.
const PO_OVERDUE_DAYS_THRESHOLD = 90;

// Tính giá trị đã giao của 1 PO (CNY) — Σ (SL đã giao × đơn giá item)
const calcPODeliveredValueCNY = (po, shipments) => {
  const items = getPOItems(po);
  let value = 0;
  items.forEach(it => {
    const shippedQty = po.items
      ? shippedFromItem(po.id, it.id, shipments)
      : shippedFromPO(po.id, shipments);
    value += shippedQty * Number(it.unitPrice || 0);
  });
  // Quy đổi sang CNY nếu PO không phải CNY (giống logic Shipment value)
  return value;
};

// Tính tổng giá trị PO (CNY) — Σ (SL đặt × đơn giá item)
const calcPOTotalValueCNY = (po) => {
  const items = getPOItems(po);
  return items.reduce((s, it) => s + Number(it.quantity || 0) * Number(it.unitPrice || 0), 0);
};

// Tính số ngày từ orderDate đến hôm nay
const daysSinceOrder = (po) => {
  if (!po.orderDate) return 0;
  const orderTime = new Date(po.orderDate).getTime();
  const now = Date.now();
  return Math.floor((now - orderTime) / (1000 * 60 * 60 * 24));
};

// Xác định trạng thái giao + nhãn
const getPODeliveryStatus = (po, shipments) => {
  if (po.status === "Hủy") return { id: "cancelled", label: "Hủy", color: C.textMuted, icon: "⚪" };
  if (!po.approved) return { id: "pending", label: "Chờ duyệt", color: C.gold, icon: "⏳" };
  const total = calcPOTotalValueCNY(po);
  const delivered = calcPODeliveredValueCNY(po, shipments);
  if (total === 0) return { id: "empty", label: "Không có SP", color: C.textMuted, icon: "—" };
  const ratio = delivered / total;
  if (ratio >= 1) return { id: "delivered", label: "Giao đủ", color: C.green600, icon: "🟢" };
  // Treo lâu = approved, chưa giao đủ, quá ngưỡng
  const days = daysSinceOrder(po);
  if (days > PO_OVERDUE_DAYS_THRESHOLD) return { id: "overdue", label: `Treo ${days}ng`, color: C.red, icon: "🔴" };
  if (ratio === 0) return { id: "not_started", label: "Chưa giao", color: C.blue, icon: "🔵" };
  return { id: "shipping", label: "Đang giao", color: C.blue, icon: "🟡" };
};

const POProgressReport = ({ pos, shipments, payments, factories, products, openingBalances, settings, user }) => {
  const [filter, setFilter] = useState({ factoryId: "", deliveryStatus: "", search: "" });
  const [dateFrom, setDateFrom] = useState("");
  const [dateTo, setDateTo] = useState("");
  ...
  // Filter PO theo date + factory + status + search
  const filteredPOs = useMemo(() => {
    return pos.filter(po => {
      if (filter.factoryId && po.factoryId !== filter.factoryId) return false;
      if (filter.search) {
        const q = filter.search.toLowerCase();
        if (!(po.id || "").toLowerCase().includes(q)) return false;
      }
      if (dateFrom && po.orderDate && po.orderDate < dateFrom) return false;
      if (dateTo && po.orderDate && po.orderDate > dateTo) return false;
      // Filter theo delivery status
      if (filter.deliveryStatus) {
        const st = getPODeliveryStatus(po, shipments);
        if (st.id !== filter.deliveryStatus) return false;
      }
      return true;
    });
  }, [pos, shipments, filter, dateFrom, dateTo]);

  // Nhóm PO theo NCC
  const groupedByFactory = useMemo(() => {
    const groups = {};
    filteredPOs.forEach(po => {
      const fid = po.factoryId || "_unknown";
  ...
  // Build dữ liệu hiển thị cho từng NCC
  const factoryRows = useMemo(() => {
    return Object.keys(groupedByFactory).map(fid => {
      const factory = factories.find(f => f.id === fid);
      const factoryName = factory?.name || "(Chưa xác định)";
      const factoryPOs = groupedByFactory[fid];
      // Tổng giá trị PO + đã giao của NCC (trong filter)
      let totalPOCNY = 0, deliveredCNY = 0;
      factoryPOs.forEach(po => {
        if (po.status === "Hủy") return;
        totalPOCNY += calcPOTotalValueCNY(po);
        deliveredCNY += calcPODeliveredValueCNY(po, shipments);
      });
      // Lấy số thanh toán THẬT từ calcFactoryBalance
      let stillOwed = 0, netPaid = 0, pendingCNY = 0;
      if (factory) {
        const bal = calcFactoryBalance(fid, pos, shipments, payments, openingBalances, factories, settings);
        stillOwed = bal.stillOwed || 0;
        netPaid = bal.netPaid || 0;
        pendingCNY = bal.pendingPaidCNY || 0;
      }
      return {
        factoryId: fid,
        factoryName,
        poCount: factoryPOs.length,
        pos: factoryPOs,
        totalPOCNY,
        deliveredCNY,
        stillOwed,
  ...
  }, [groupedByFactory, factories, pos, shipments, payments, openingBalances, settings]);

  // Tổng hợp toàn báo cáo
  const summary = useMemo(() => {
    let totalPOCNY = 0, deliveredCNY = 0, paidCNY = 0, pendingCNY = 0, overdueCount = 0;
    factoryRows.forEach(fr => {
      totalPOCNY += fr.totalPOCNY;
      deliveredCNY += fr.deliveredCNY;
      paidCNY += fr.netPaid;
      pendingCNY += fr.pendingCNY;
    });
    filteredPOs.forEach(po => {
      const st = getPODeliveryStatus(po, shipments);
      if (st.id === "overdue") overdueCount++;
    });
    return { totalPOCNY, deliveredCNY, paidCNY, pendingCNY, overdueCount, poCount: filteredPOs.length };
  }, [factoryRows, filteredPOs, shipments]);

  const cnyToVnd = settings?.cnyToVnd || 1;

  const SummaryCard = ({ label, sub, cny, color, hint }) => (
    <div style={{ flex: 1, minWidth: 180, background: C.white, border: `1px solid ${C.border}`, borderRadius: 14, padding: "14px 18px" }}>
      ...
      {cny !== undefined && (
        <>
          <div style={{ fontSize: 20, fontWeight: 800, color, marginTop: 6 }}>{fmt(cny, "CNY")}</div>
          <div style={{ fontSize: 11, color: C.textMuted, marginTop: 2 }}>≈ {fmt(cny * cnyToVnd, "VND")}</div>
        </>
      )}
      {hint && <div style={{ fontSize: 22, fontWeight: 800, color, marginTop: 6 }}>{hint}</div>}
    </div>
  );

  const handleExport = async () => {
    try {
      await exportPOProgressReport({
        factoryRows, summary, shipments, products,
        filterFactoryName: factories.find(f => f.id === filter.factoryId)?.name || "",
  ...
  return (
    <div>
      ...
      <div className="alert alert-info" style={{ marginBottom: 14, fontSize: 12, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
        <div>
          <b>📊 Báo cáo tiến độ PO — nhóm theo NCC</b>
          <div style={{ marginTop: 4, fontSize: 11, lineHeight: 1.5 }}>
            💡 Công nợ chỉ phát sinh khi <b>HÀNG ĐÃ GIAO</b>. Tiền "Đang treo" (uỷ thác + đang chuyển QT) tách riêng khỏi "Đã trả".
            <br />Click NCC để xem các PO. Click PO để xem chi tiết SP.
          </div>
        </div>
        {canExport && (
          <button className="btn btn-ghost" style={{ padding: "6px 12px", fontSize: 12, whiteSpace: "nowrap" }} onClick={handleExport}>📥 Xuất Excel 3 sheets</button>
        )}
      </div>

      {/* Summary bar */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 12, marginBottom: 14 }}>
        <SummaryCard label="📦 Tổng giá trị PO" sub={`${summary.poCount} PO`} cny={summary.totalPOCNY} color={C.text} />
        <SummaryCard label="🚚 Đã giao" sub={summary.totalPOCNY > 0 ? `${((summary.deliveredCNY / summary.totalPOCNY) * 100).toFixed(0)}% giá trị PO` : "—"} cny={summary.deliveredCNY} color={C.blue} />
        <SummaryCard label="✅ Đã thanh toán" sub="Stage Hoàn tất" cny={summary.paidCNY} color={C.green600} />
        <SummaryCard label="🌐 Đang treo" sub="Uỷ thác + Đang chuyển QT" cny={summary.pendingCNY} color={C.gold} />
        <div style={{ minWidth: 180, background: summary.overdueCount > 0 ? "#FEF2F2" : C.white, border: `1px solid ${summary.overdueCount > 0 ? C.red : C.border}`, borderRadius: 14, padding: "14px 18px" }}>
          <div style={{ fontSize: 11, color: C.textMuted, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.04em" }}>🔴 Treo lâu</div>
          <div style={{ fontSize: 10, color: C.textLight, marginTop: 1 }}>{`> ${PO_OVERDUE_DAYS_THRESHOLD} ngày chưa đủ`}</div>
          <div style={{ fontSize: 22, fontWeight: 800, color: summary.overdueCount > 0 ? C.red : C.textMuted, marginTop: 6 }}>{summary.overdueCount}</div>
          <div style={{ fontSize: 11, color: C.textMuted, marginTop: 2 }}>PO cần xử lý</div>
        </div>
      </div>

      {/* Filters */}
      <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 16, padding: "14px 18px", marginBottom: 14, display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
        <select style={{ width: 220 }} value={filter.factoryId} onChange={e => setFilter(p => ({ ...p, factoryId: e.target.value }))}>
          <option value="">🏭 Tất cả NCC</option>
          {factories.map(f => <option key={f.id} value={f.id}>{f.name}</option>)}
        </select>
        <select style={{ width: 200 }} value={filter.deliveryStatus} onChange={e => setFilter(p => ({ ...p, deliveryStatus: e.target.value }))}>
          <option value="">🚚 Tất cả trạng thái giao</option>
          <option value="pending">⏳ Chờ duyệt</option>
          <option value="not_started">🔵 Chưa giao</option>
          <option value="shipping">🟡 Đang giao</option>
          <option value="delivered">🟢 Giao đủ</option>
          <option value="overdue">🔴 Treo lâu</option>
          <option value="cancelled">⚪ Hủy</option>
        </select>
        <input placeholder="🔍 Tìm theo mã PO..." value={filter.search} onChange={e => setFilter(p => ({ ...p, search: e.target.value }))} style={{ flex: 1, minWidth: 180 }} />
        ...
        {(filter.factoryId || filter.deliveryStatus || filter.search || dateFrom || dateTo) && (
          <button className="btn btn-ghost" style={{ fontSize: 11, padding: "4px 10px" }} onClick={() => { setFilter({ factoryId: "", deliveryStatus: "", search: "" }); setDateFrom(""); setDateTo(""); }}>✕ Xóa lọc</button>
        )}
      </div>

      {/* Main table — 3-level grouping */}
      <div className="card" style={{ padding: 0, overflow: "auto" }}>
        {factoryRows.length === 0 ? (
          <div style={{ padding: 40, textAlign: "center", color: C.textMuted }}>Không có PO nào khớp bộ lọc</div>
        ) : (
          <table style={{ minWidth: 1200 }}>
            <thead>
              <tr>
                ...
                <th>NCC / PO</th>
                <th style={{ textAlign: "right" }}>Giá trị PO</th>
                <th style={{ minWidth: 220 }}>Tiến độ giao</th>
                <th style={{ textAlign: "right" }}>Đã thanh toán</th>
                <th style={{ textAlign: "right" }}>Đang treo</th>
                <th style={{ textAlign: "right" }}>Còn phải trả</th>
                <th>Trạng thái</th>
              </tr>
            </thead>
            <tbody>
              {factoryRows.map(fr => {
                const isExpanded = expandedFactory[fr.factoryId];
                const deliveredPct = fr.totalPOCNY > 0 ? (fr.deliveredCNY / fr.totalPOCNY) * 100 : 0;
                return (
                  <Fragment key={fr.factoryId}>
                    {/* NCC row */}
                    <tr style={{ background: isExpanded ? C.green50 : "transparent", cursor: "pointer", fontWeight: 600 }}
                      onClick={() => setExpandedFactory(p => ({ ...p, [fr.factoryId]: !p[fr.factoryId] }))}>
                      <td style={{ textAlign: "center" }}>{isExpanded ? "▼" : "▶"}</td>
                      <td>
                        <div style={{ fontWeight: 700, color: C.green700, fontSize: 14 }}>🏭 {fr.factoryName}</div>
                        <div style={{ fontSize: 11, color: C.textMuted, fontWeight: 500 }}>{fr.poCount} PO</div>
                      </td>
                      <td style={{ textAlign: "right" }}>
                        <div style={{ fontWeight: 700 }}>{fmt(fr.totalPOCNY, "CNY")}</div>
                        <div style={{ fontSize: 10, color: C.textMuted, fontWeight: 500 }}>≈ {fmt(fr.totalPOCNY * cnyToVnd, "VND")}</div>
                      </td>
                      <td>
                        <div style={{ height: 8, background: C.bg, borderRadius: 4, overflow: "hidden" }}>
                          <div style={{ width: `${Math.min(100, deliveredPct)}%`, height: "100%", background: deliveredPct >= 100 ? C.green600 : C.blue }}></div>
                        </div>
                        <div style={{ fontSize: 11, color: C.textMuted, marginTop: 3, fontWeight: 500 }}>{deliveredPct.toFixed(0)}% · {fmt(fr.deliveredCNY, "CNY")}</div>
                      </td>
                      <td style={{ textAlign: "right" }}>
                        <div style={{ color: C.green600, fontWeight: 700 }}>{fmt(fr.netPaid, "CNY")}</div>
                        <div style={{ fontSize: 10, color: C.textMuted, fontWeight: 500 }}>≈ {fmt(fr.netPaid * cnyToVnd, "VND")}</div>
                      </td>
                      <td style={{ textAlign: "right" }}>
                        <div style={{ color: fr.pendingCNY > 0 ? C.gold : C.textLight, fontWeight: 700 }}>{fmt(fr.pendingCNY, "CNY")}</div>
                        <div style={{ fontSize: 10, color: C.textMuted, fontWeight: 500 }}>≈ {fmt(fr.pendingCNY * cnyToVnd, "VND")}</div>
                      </td>
                      <td style={{ textAlign: "right" }}>
                        <div style={{ color: fr.stillOwed > 0 ? C.red : C.green600, fontWeight: 700 }}>{fmt(fr.stillOwed, "CNY")}</div>
                        <div style={{ fontSize: 10, color: C.textMuted, fontWeight: 500 }}>≈ {fmt(fr.stillOwed * cnyToVnd, "VND")}</div>
                      </td>
                      <td>
                        <span style={{ fontSize: 11, color: C.textMuted, fontWeight: 500 }}>(Mức NCC)</span>
                      </td>
                    </tr>

                    {/* PO rows under this factory */}
                    {isExpanded && fr.pos.map(po => {
                      const isPoExpanded = expandedPO[po.id];
                      const poTotal = calcPOTotalValueCNY(po);
                      const poDelivered = calcPODeliveredValueCNY(po, shipments);
                      const pctOfPO = poTotal > 0 ? (poDelivered / poTotal) * 100 : 0;
                      const st = getPODeliveryStatus(po, shipments);
                      const poItems = getPOItems(po);
                      return (
                        <Fragment key={po.id}>
                          <tr style={{ background: isPoExpanded ? "#F7FAF7" : "transparent", cursor: "pointer" }}
                            onClick={() => setExpandedPO(p => ({ ...p, [po.id]: !p[po.id] }))}>
                            <td></td>
                            <td style={{ paddingLeft: 32 }}>
                              <div style={{ fontSize: 12 }}>
                                <span style={{ color: C.textMuted, marginRight: 4 }}>{isPoExpanded ? "▼" : "▶"}</span>
                                <span style={{ fontWeight: 700, color: C.green600 }}>{po.id}</span>
                              </div>
                              <div style={{ fontSize: 10, color: C.textMuted, marginLeft: 18 }}>{po.orderDate || "—"} · {poItems.length} SP</div>
                            </td>
                            <td style={{ textAlign: "right", fontSize: 12 }}>{fmt(poTotal, "CNY")}</td>
                            <td>
                              <div style={{ height: 6, background: C.bg, borderRadius: 3, overflow: "hidden" }}>
                                <div style={{ width: `${Math.min(100, pctOfPO)}%`, height: "100%", background: pctOfPO >= 100 ? C.green600 : C.blue }}></div>
                              </div>
                              <div style={{ fontSize: 10, color: C.textMuted, marginTop: 2 }}>{pctOfPO.toFixed(0)}% PO · {fmt(poDelivered, "CNY")}</div>
                            </td>
                            <td colSpan={3} style={{ textAlign: "center", color: C.textLight, fontSize: 11, fontStyle: "italic" }}>(Thanh toán chỉ tính ở mức NCC)</td>
                            <td>
                              <span style={{ display: "inline-block", padding: "3px 8px", borderRadius: 6, fontSize: 11, fontWeight: 700, color: st.color, background: `${st.color}15`, border: `1px solid ${st.color}` }}>{st.icon} {st.label}</span>
                            </td>
                          </tr>

                          {/* SP rows under this PO */}
                          {isPoExpanded && (
                            <tr>
                              <td></td>
                              <td colSpan={7} style={{ background: "#FAFCFA", padding: "10px 20px 14px 60px" }}>
                                <table style={{ width: "100%", fontSize: 11 }}>
                                  <thead>
                                    <tr>
                                      ...
                                      <th style={{ textAlign: "right" }}>Đã giao</th>
                                      <th style={{ textAlign: "right" }}>Còn lại</th>
                                      <th style={{ textAlign: "right" }}>Đơn giá</th>
                                      <th style={{ textAlign: "right" }}>Giá trị đặt</th>
                                      <th style={{ textAlign: "right" }}>Giá trị giao</th>
                                    </tr>
                                  </thead>
                                  <tbody>
                                    {poItems.map(it => {
                                      ...
                                      return (
                                        <tr key={it.id}>
                                          <td style={{ fontWeight: 600, color: C.green600 }}>{prod?.sku || "—"}</td>
                                          <td>{prod?.name || "—"}</td>
                                          <td style={{ textAlign: "right", fontWeight: 600 }}>{Number(it.quantity).toLocaleString()}</td>
                                          <td style={{ textAlign: "right", color: C.blue, fontWeight: 600 }}>{shipped.toLocaleString()}</td>
                                          <td style={{ textAlign: "right", color: remain > 0 ? C.orange : C.green600, fontWeight: 600 }}>{remain.toLocaleString()}</td>
                                          <td style={{ textAlign: "right" }}>{fmt(it.unitPrice, po.currency)}</td>
                                          <td style={{ textAlign: "right", fontWeight: 600 }}>{fmt(itemTotal, po.currency)}</td>
                                          <td style={{ textAlign: "right", color: C.blue, fontWeight: 600 }}>{fmt(itemDelivered, po.currency)}</td>
                                        </tr>
                                      );
                                    })}
                                  </tbody>
...
const POs = ({ pos, factories, products, shipments, markets = [], settings, onAdd, onEdit, onDelete, onHardDelete, onRenameId, data, onConfirm, prefill, onClearPrefill, user }) => {
  ...
  // v38m: Sub-tab — "list" (danh sách PO cũ) | "report" (báo cáo tiến độ mới)
  ...
  const filtered = useMemo(() => {
    ...
    const matched = filterByDateRange(pos, "orderDate", dateFrom, dateTo).filter(p => {
      ...
      if (!matchStatusFilter(filter.status, p.status)) return false;
  ...
  return (
    <div>
      <SectionHeader title="Đơn đặt hàng" subtitle={`Click vào PO để xem chi tiết${pendingCount > 0 ? ` · ${pendingCount} PO chờ duyệt` : ""}`}
        action={
          <div style={{ display: "flex", gap: 8 }}>
            {/* v37: Nút xuất báo cáo PO chi tiết — chỉ hiện ở sub-tab list */}
            {canExport && subTab === "list" && (
            ...
            {canCreate && subTab === "list" && <button className="btn btn-primary" onClick={() => setModal({ type: "new" })}>+ Tạo PO mới</button>}
          </div>
        }
      />
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 14, borderBottom: `1px solid ${C.borderLight}` }}>
        <button onClick={() => setSubTab("list")} className="btn btn-ghost"
          style={{ padding: "8px 14px", fontSize: 13, fontWeight: subTab === "list" ? 700 : 500,
            borderBottom: subTab === "list" ? `2px solid ${C.green600}` : "2px solid transparent",
            borderRadius: 0, color: subTab === "list" ? C.green700 : C.textMuted }}>📋 Danh sách PO</button>
        <button onClick={() => setSubTab("report")} className="btn btn-ghost"
          style={{ padding: "8px 14px", fontSize: 13, fontWeight: subTab === "report" ? 700 : 500,
            borderBottom: subTab === "report" ? `2px solid ${C.green600}` : "2px solid transparent",
            borderRadius: 0, color: subTab === "report" ? C.green700 : C.textMuted }}>📊 Báo cáo tiến độ <span style={{ fontSize: 10, marginLeft: 4, color: C.orange }}>(V38m)</span></button>
      </div>

      {/* v38m: Sub-tab "report" — báo cáo tiến độ PO */}
      {subTab === "report" && (
        <POProgressReport
          pos={pos}
          shipments={shipments}
      ...
      {/* v38m: Sub-tab "list" — bọc toàn bộ nội dung POs cũ */}
      {subTab === "list" && (<>
...
const Shipments = ({ shipments, pos, factories, products, feePayments, markets, carriers, settings, onAdd, onEdit, onDelete, onHardDelete, onRenameId, data, onCreateWarehouse, prefill, onClearPrefill, user }) => {
  ...
  const [filter, setFilter] = useState({ market: "", warehouse: "", status: [], factory: "", search: "" });
  const [dateFrom, setDateFrom] = useState("");
  const [dateTo, setDateTo] = useState("");
  const [expanded, setExpanded] = useState(null);
  ...
  const filtered = useMemo(() => {
    ...
    const matched = filterByDateRange(shipments, "departDate", dateFrom, dateTo).filter(s => {
      ...
      if (!matchStatusFilter(filter.status, s.status)) return false;
      // v38n: Lọc theo NCC — shipment chứa item có poId thuộc PO của NCC này
      if (filter.factory) {
        const hasFactoryItem = (s.items || []).some(it => {
          const po = pos.find(p => p.id === it.poId);
          return po?.factoryId === filter.factory;
        });
        if (!hasFactoryItem) return false;
      }
      if (!q) return true;
  ...
  // v38k: Tách helper tính giá trị 1 shipment (CNY) — dùng cho cả summary lẫn cột Giá trị từng dòng.
  const calcShipmentValueCNY = (s) => {
    let valCNY = 0;
    (s.items || []).forEach(i => {
      const po = pos.find(p => p.id === i.poId);
      if (!po) return;
  ...
  const summary = useMemo(() => {
    ...
    const inTransitStatuses = ["Đang vận chuyển TQ", "Đang vận chuyển", "Đang xếp tàu", "Đang thông quan", "Kiểm hoá", "Đã thông quan"];
    filtered.forEach(s => {
      ...
      // v38k: Dùng helper chung
      totalValueCNY += calcShipmentValueCNY(s);
    });
    const cnyToVnd = settings?.cnyToVnd || 1;
    return {
      count: filtered.length,
    ...
    <div>
      ...
      <SummaryBar
        ...
        items={[
          ...
          { icon: "💰", label: "Tổng giá trị", primary: fmt(summary.totalCNY, "CNY"), secondary: `≈ ${fmt(summary.totalVND, "VND")}`, color: C.green700 },
      ...
      <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 16, padding: "14px 18px", marginBottom: 16, display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <MultiSelectStatus
          ...
          label="Tất cả trạng thái"
          width={210}
          renderColor={shipmentStatusColor}
        />
        {/* v38n: Lọc theo NCC */}
        <select style={{ width: 200 }} value={filter.factory} onChange={e => setFilter(p => ({ ...p, factory: e.target.value }))} title="Chỉ hiện shipment có hàng từ NCC này">
          <option value="">🏭 Tất cả NCC</option>
          {factories.map(f => <option key={f.id} value={f.id}>{f.name}</option>)}
        </select>
        <div style={{ flex: 1, minWidth: 300 }}>
      ...
      <div className="card" style={{ padding: 0, overflow: "hidden" }}>
        <table>
          <thead><tr><th></th><th>Mã đơn giao hàng</th><th>Thị trường / Kho</th><th>Trạng thái</th><th>Ngày xuất → nhận</th><th>Carrier</th><th>Tracking</th><th>Kiện</th><th>SL</th><th>Giá trị</th><th>📄</th><th></th></tr></thead>
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={12} style={{ textAlign: "center", color: C.textMuted, padding: 40 }}>Không có đơn giao hàng nào</td></tr>
            ) : pagedFiltered.map(s => {
              ...
              return (
                <Fragment key={s.id}>
                  <tr className={isExpanded ? "expanded" : ""} onClick={() => setExpanded(isExpanded ? null : s.id)} style={{ cursor: "pointer", opacity: s.status === "Hủy" ? 0.6 : 1 }}>
                    ...
                    {/* v38k: Cột Giá trị hàng — ¥ chính + quy đổi VNĐ nhỏ bên dưới */}
                    <td style={{ fontWeight: 600 }}>
                      {(() => {
                        const valCNY = calcShipmentValueCNY(s);
                        const valVND = valCNY * (settings?.cnyToVnd || 1);
                        return (
                          <>
                            <div style={{ color: C.green700, fontSize: 13 }}>{fmt(valCNY, "CNY")}</div>
                            <div style={{ fontSize: 10, color: C.textMuted, fontWeight: 500 }}>≈ {fmt(valVND, "VND")}</div>
                          </>
                        );
                      })()}
                    </td>
                  ...
                  {isExpanded && (
                    <tr>
                      <td colSpan={12} style={{ background: C.green50, padding: 24 }}>
...
const Warranties = ({ warranties = [], factories, markets, products, settings, onAdd, onEdit, onDelete, user }) => {
  ...
  const filtered = useMemo(() => {
    const matched = warranties.filter(w => {
      if (!matchStatusFilter(filter.status, w.status)) return false;
  ...
  return (
    <div>
      ...
      <div className="card" style={{ padding: "12px 16px", marginBottom: 16, display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
        ...
        {(filter.status?.length > 0 || filter.search || filter.dateFrom || filter.dateTo) && (
...
const MarketDebts = ({ pos, shipments, payments, factories, markets, settings, products, warranties = [], openingBalances = [], user }) => {
  ...
  // v38n: Filter kỳ — Cách A. Khi có filter → hiện PS nợ/PS trả/Chênh lệch trong kỳ.
  // Khi không filter → giữ nguyên view tích lũy như cũ.
  ...
  // v38n: Tính PS trong kỳ cho từng TT (chỉ khi có filter)
  const periodBalances = useMemo(() => {
    if (!hasPeriodFilter) return null;
    const res = {};
    marketNames.forEach(m => {
      res[m] = calcMarketBalanceInPeriod(m, periodFrom, periodTo, pos, shipments, payments, warranties, products, settings);
    });
    return res;
  }, [hasPeriodFilter, periodFrom, periodTo, pos, shipments, payments, warranties, products, settings]);

  // v38n: Summary trong kỳ
  const periodSummary = useMemo(() => {
    if (!periodBalances) return null;
    let psNo = 0, psTra = 0, psPending = 0, psWarranty = 0;
    marketNames.forEach(m => {
      psNo += periodBalances[m].psNoCNY;
      psTra += periodBalances[m].psTraCNY;
      psPending += periodBalances[m].psPendingCNY;
      psWarranty += periodBalances[m].psWarrantyCNY;
    });
    return { psNo, psTra, psPending, psWarranty, chenhLech: psNo + psWarranty - psTra };
  }, [periodBalances]);
  ...
  return (
    <div>
      ...
      {/* v38n: Filter kỳ — Cách A */}
      <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 14, padding: "12px 16px", marginBottom: 16, display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
        <span style={{ fontSize: 12, color: C.textMuted, fontWeight: 600 }}>📅 Lọc kỳ:</span>
        <input type="date" value={periodFrom} onChange={e => setPeriodFrom(e.target.value)} title="Từ ngày" />
        <span style={{ color: C.textMuted }}>→</span>
        <input type="date" value={periodTo} onChange={e => setPeriodTo(e.target.value)} title="Đến ngày" />
        {hasPeriodFilter && (
          <>
            <button className="btn btn-ghost" style={{ padding: "4px 10px", fontSize: 11 }} onClick={() => { setPeriodFrom(""); setPeriodTo(""); }}>✕ Bỏ lọc</button>
            <span style={{ fontSize: 11, padding: "4px 10px", background: C.blueBg, color: C.blue, borderRadius: 8, fontWeight: 600 }}>
              📊 Đang xem PHÁT SINH TRONG KỲ {periodFrom || "..."} → {periodTo || "..."}
            </span>
          </>
        )}
        {!hasPeriodFilter && (
          <span style={{ fontSize: 11, color: C.textLight, fontStyle: "italic" }}>(Để trống = xem toàn bộ tích lũy như hiện tại)</span>
        )}
      </div>

      {/* v38n: Khi có filter kỳ → hiện KPI PS trong kỳ */}
      {hasPeriodFilter && periodSummary && (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 10, marginBottom: 20 }}>
          {[
            { label: "+ PS Nợ trong kỳ", val: periodSummary.psNo, color: C.orange, sub: "Σ hàng ship trong kỳ" },
            { label: "− PS Trả trong kỳ", val: periodSummary.psTra, color: C.green600, sub: "Σ TT hoàn tất trong kỳ" },
            { label: "+ BH treo trong kỳ", val: periodSummary.psWarranty, color: C.purple, sub: "Σ BH ship trong kỳ" },
            { label: "= Chênh lệch trong kỳ", val: periodSummary.chenhLech, color: periodSummary.chenhLech > 0 ? C.red : C.green600, sub: periodSummary.chenhLech > 0 ? "Nợ tăng trong kỳ" : "Nợ giảm trong kỳ" },
            { label: "🟡 Đang treo trong kỳ", val: periodSummary.psPending, color: C.orange, sub: "Payment đang chuyển/uỷ thác" },
          ].map((k, i) => (
      ...
      {/* v38: KPI 6 thẻ — thêm "🟡 Đang TT" giữa "Đã thanh toán" và "Hàng đang BH" — CHỈ HIỆN KHI KHÔNG FILTER KỲ */}
      {!hasPeriodFilter && (
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        {marketNames.map(m => {
          const b = balances[m];
          const pb = periodBalances?.[m]; // v38n: PS trong kỳ (nếu có filter)
            ...
            <div key={m} className="card">
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", cursor: "pointer", flexWrap: "wrap", gap: 12 }} onClick={() => setExpanded(isExpanded ? null : m)}>
                ...
                <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                  {/* v38n: Khi có filter kỳ → hiện PS trong kỳ thay cho các KPI tích lũy. */}
                  {hasPeriodFilter && pb ? (
                    <>
                      <div style={{ textAlign: "right" }}>
                        <div style={{ fontSize: 10, color: C.orange, textTransform: "uppercase", fontWeight: 600 }}>+ PS Nợ</div>
                        <div style={{ fontWeight: 700, color: C.orange }}>{fmt(pb.psNoCNY, "CNY")}</div>
                        <div style={{ fontSize: 10, color: C.textMuted }}>≈ {fmt(toVND(pb.psNoCNY, "CNY", settings), "VND")}</div>
                      </div>
                      <div style={{ textAlign: "right" }}>
                        <div style={{ fontSize: 10, color: C.green600, textTransform: "uppercase", fontWeight: 600 }}>− PS Trả</div>
                        <div style={{ fontWeight: 700, color: C.green600 }}>{fmt(pb.psTraCNY, "CNY")}</div>
                        <div style={{ fontSize: 10, color: C.textMuted }}>≈ {fmt(toVND(pb.psTraCNY, "CNY", settings), "VND")}</div>
                      </div>
                      {pb.psWarrantyCNY > 0 && (
                        <div style={{ textAlign: "right" }}>
                          <div style={{ fontSize: 10, color: C.purple, textTransform: "uppercase", fontWeight: 600 }}>+ BH treo</div>
                          <div style={{ fontWeight: 700, color: C.purple }}>{fmt(pb.psWarrantyCNY, "CNY")}</div>
                          <div style={{ fontSize: 10, color: C.textMuted }}>≈ {fmt(toVND(pb.psWarrantyCNY, "CNY", settings), "VND")}</div>
                        </div>
                      )}
                      {pb.psPendingCNY > 0 && (
                        <div style={{ textAlign: "right" }}>
                          <div style={{ fontSize: 10, color: C.orange, textTransform: "uppercase", fontWeight: 600 }}>🟡 Đang treo</div>
                          <div style={{ fontWeight: 700, color: C.orange }}>{fmt(pb.psPendingCNY, "CNY")}</div>
                          <div style={{ fontSize: 10, color: C.textMuted }}>≈ {fmt(toVND(pb.psPendingCNY, "CNY", settings), "VND")}</div>
                        </div>
                      )}
                      <div style={{ textAlign: "right", padding: "8px 14px", background: pb.chenhLechCNY > 0 ? C.redBg : C.green50, borderRadius: 10, minWidth: 160 }}>
                        <div style={{ fontSize: 10, color: C.textMuted, textTransform: "uppercase", fontWeight: 600 }}>= Chênh lệch</div>
                        <div style={{ fontWeight: 700, color: pb.chenhLechCNY > 0 ? C.red : C.green600 }}>
                          {pb.chenhLechCNY > 0 ? "+" : ""}{fmt(pb.chenhLechCNY, "CNY")}
                        </div>
                        <div style={{ fontSize: 10, color: C.textMuted }}>≈ {fmt(toVND(pb.chenhLechCNY, "CNY", settings), "VND")}</div>
                      </div>
                    </>
                  ) : (
                  <>
...
const Payments = ({ pos, shipments, payments, factories, openingBalances, markets, settings, onAdd, onEdit, onDelete, user }) => {
  ...
  // v38n: Thêm filter market (chỉ áp dụng cho MARKET_TO_FACTORY)
  const [filter, setFilter] = useState({ type: "", factory: "", stage: [], market: "", showCancelled: true });
  const [dateFrom, setDateFrom] = useState("");
  const [dateTo, setDateTo] = useState("");
  ...
  const filtered = useMemo(() => {
    const matched = filterByDateRange(payments, "payDate", dateFrom, dateTo).filter(p =>
      ...
      matchStatusFilter(filter.stage, getPaymentStage(p)) &&
      // v38n: Filter theo thị trường — chỉ áp dụng cho MARKET_TO_FACTORY (có field fromMarket).
      // Payment INTER_FACTORY không có fromMarket → khi filter bị áp dụng, INTER bị ẩn (đúng logic).
      (!filter.market || p.fromMarket === filter.market) &&
  ...
  return (
    <div>
      ...
      {/* v38L: Thanh Tổng hợp thanh toán — số GD, tổng hoàn tất, tổng đang chuyển */}
      {(() => {
        // Tính trên danh sách đã filter (không tính các lô đã hủy)
        const active = filtered.filter(p => p.status !== "cancelled");
        const cnyOf = (p) => {
          // Quy về CNY: nếu currency = CNY dùng thẳng, nếu khác quy đổi qua VND
          if ((p.currency || "CNY") === "CNY") return Number(p.amount || 0);
          const vnd = p.amountInVND ?? toVND(p.amount, p.currency, settings);
          return vnd / (settings?.cnyToVnd || 1);
        };
        let completedCNY = 0, pendingCNY = 0;
        active.forEach(p => {
          const stage = getPaymentStage(p);
          const val = cnyOf(p);
          if (stage === "completed") completedCNY += val;
          else if (PAYMENT_PENDING_STAGES.includes(stage)) pendingCNY += val;
        });
        const cnyToVnd = settings?.cnyToVnd || 1;
        const Card = ({ label, sub, cny, color }) => (
          <div style={{ flex: 1, minWidth: 200, background: C.white, border: `1px solid ${C.border}`, borderRadius: 14, padding: "14px 18px" }}>
            <div style={{ fontSize: 11, color: C.textMuted, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.04em" }}>{label}</div>
            {sub && <div style={{ fontSize: 10, color: C.textLight, marginTop: 1 }}>{sub}</div>}
            {cny !== undefined ? (
              <>
        ...
        return (
          <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 16 }}>
            <div style={{ flex: 1, minWidth: 160, background: C.white, border: `1px solid ${C.border}`, borderRadius: 14, padding: "14px 18px" }}>
              <div style={{ fontSize: 11, color: C.textMuted, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.04em" }}>Số giao dịch TT</div>
              <div style={{ fontSize: 20, fontWeight: 800, color: C.text, marginTop: 6 }}>{active.length}</div>
              <div style={{ fontSize: 11, color: C.textMuted, marginTop: 2 }}>giao dịch (đang lọc)</div>
            </div>
            <Card label="✅ Tổng Hoàn tất" sub="Stage Hoàn tất thanh toán" cny={completedCNY} color={C.green600} />
            <Card label="🌐 Tổng Đang chuyển" sub="Đang chuyển QT + Đã chuyển UT" cny={pendingCNY} color={C.blue} />
          </div>
        );
      })()}
      ...
      <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 16, padding: "14px 18px", marginBottom: 16, display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
        ...
        {/* v38: Filter theo stage thanh toán — v38o: multi-select */}
        {(() => {
          const stageList = Object.values(PAYMENT_STAGES);
          // Map label "icon label" ↔ id (vì MultiSelectStatus dùng string làm value)
          const labelOf = (s) => `${s.icon} ${s.label}`;
          const allLabels = stageList.map(labelOf);
          const idOf = (label) => stageList.find(s => labelOf(s) === label)?.id;
          const labelByIdOf = (id) => labelOf(stageList.find(s => s.id === id) || { icon: "", label: id });
          const valueLabels = Array.isArray(filter.stage)
            ? filter.stage.filter(v => v !== "__NONE__").map(labelByIdOf)
            : [];
          const noneMarker = Array.isArray(filter.stage) && filter.stage.includes("__NONE__");
          return (
            <MultiSelectStatus
              options={allLabels}
              value={noneMarker ? ["__NONE__"] : valueLabels}
              onChange={(newLabels) => {
                if (newLabels.includes("__NONE__")) setFilter(p => ({ ...p, stage: ["__NONE__"] }));
                else setFilter(p => ({ ...p, stage: newLabels.map(idOf).filter(Boolean) }));
              }}
              label="Tất cả trạng thái"
              width={220}
            />
        ...
        {/* v38n: Filter theo thị trường — chỉ có nghĩa với payment Thị trường → Nhà máy */}
        <select style={{ width: 180 }} value={filter.market} onChange={e => setFilter(p => ({ ...p, market: e.target.value }))} title="Chỉ áp dụng cho TT → NM (loại Liên nhà máy sẽ bị ẩn khi lọc)">
          <option value="">🌍 Tất cả thị trường</option>
          {marketNames.map(m => <option key={m} value={m}>{getFlag(m)} {m}</option>)}
        </select>
...
const AuditLog = ({ auditLog }) => {
  ...
  // v38L: Theo dõi dòng log nào đang mở rộng chi tiết
  ...
  // v38L: Lấy debtImpact từ detail của log (nếu có)
  const getDebtImpact = (log) => {
    if (!log.detail) return null;
    try {
      const obj = typeof log.detail === "string" ? JSON.parse(log.detail) : log.detail;
      if (obj && Array.isArray(obj.debtImpact) && obj.debtImpact.length > 0) return obj.debtImpact;
      return null;
    } catch {
      return null;
    }
  ...
  // v38L: Format CNY ngắn cho log
  const fmtCNYShort = (n) => `${Math.round(Number(n) || 0).toLocaleString("vi-VN")} ¥`;

  // v38L: Build mô tả tác động công nợ dạng text (để rút gọn 50 ký tự)
  const buildImpactSummary = (debtImpact) => {
    if (!debtImpact || debtImpact.length === 0) return "";
    const parts = [];
    debtImpact.forEach(imp => {
      if (!imp.changed) {
        parts.push(`${imp.factoryName}: không ảnh hưởng công nợ`);
      } else {
        const changedLines = imp.lines.filter(l => l.delta !== 0)
          .map(l => `${l.label} ${fmtCNYShort(l.before)}→${fmtCNYShort(l.after)}`);
        parts.push(`${imp.factoryName}: ${changedLines.join(", ")}`);
      }
    });
    return parts.join(" · ");
  };
  ...
  return (
    <div>
      ...
      <div className="card" style={{ padding: 0, overflow: "hidden" }}>
        <table>
          <thead><tr><th>Thời gian</th><th>Người dùng</th><th>Hành động</th><th>Đối tượng</th><th>Chi tiết</th><th>Tác động công nợ NCC</th></tr></thead>
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={6} style={{ textAlign: "center", color: C.textMuted, padding: 40 }}>Chưa có hoạt động nào</td></tr>
            ) : pagedFiltered.map(log => {
              const keyDetails = extractKeyDetail(log);
              const debtImpact = getDebtImpact(log);
              const impactSummary = buildImpactSummary(debtImpact);
              const isExpanded = expandedLog === log.id;
              const isLong = impactSummary.length > 50;
              const shortSummary = isLong ? impactSummary.slice(0, 50) + "…" : impactSummary;
              return (
                <tr key={log.id}>
                  ...
                  <td style={{ fontSize: 11, color: C.textMuted, maxWidth: 360 }}>
                    {keyDetails ? (
                  ...
                  {/* v38L: Cột tác động công nợ NCC */}
                  <td style={{ fontSize: 11, maxWidth: 380 }}>
                    {!debtImpact ? (
                      <span style={{ color: C.textLight }}>—</span>
                    ) : !isExpanded ? (
                      <div>
                        <span style={{ color: C.textMuted }}>{shortSummary}</span>
                        {isLong && (
                          <button
                            onClick={() => setExpandedLog(log.id)}
                            style={{ marginLeft: 4, padding: 0, border: "none", background: "none", color: C.green600, cursor: "pointer", fontSize: 11, fontWeight: 600, textDecoration: "underline" }}
                          >xem thêm</button>
                        )}
                      </div>
                    ) : (
                      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                        {debtImpact.map((imp, i) => (
                          <div key={i} style={{ background: C.green50, borderRadius: 8, padding: "8px 10px", border: `1px solid ${C.green100}` }}>
                            <div style={{ fontWeight: 700, color: C.green700, marginBottom: 4 }}>🏭 {imp.factoryName}</div>
                            {!imp.changed ? (
                              <div style={{ color: C.textMuted, fontStyle: "italic" }}>Không ảnh hưởng công nợ NCC</div>
                            ) : (
                              <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
                                {imp.lines.map((l, j) => {
                                  const noChange = l.delta === 0;
                                  const deltaColor = l.delta > 0 ? C.red : l.delta < 0 ? C.green600 : C.textLight;
                                  return (
                                    <div key={j} style={{ display: "flex", justifyContent: "space-between", gap: 8 }}>
                                      <span style={{ color: C.textMuted }}>{l.label}:</span>
                                      <span>
                                        <span style={{ color: C.textLight }}>{fmtCNYShort(l.before)}</span>
                                        <span style={{ color: C.textMuted }}> → </span>
                                        <span style={{ fontWeight: 700, color: noChange ? C.textMuted : C.text }}>{fmtCNYShort(l.after)}</span>
                                        {!noChange && (
                                          <span style={{ color: deltaColor, fontWeight: 600, marginLeft: 4 }}>
                                            ({l.delta > 0 ? "+" : ""}{fmtCNYShort(l.delta)})
                                          </span>
                                        )}
                                      </span>
                                    </div>
                        ...
                        <button
                          ...
                          style={{ alignSelf: "flex-start", padding: 0, border: "none", background: "none", color: C.green600, cursor: "pointer", fontSize: 11, fontWeight: 600, textDecoration: "underline" }}
                        >thu gọn</button>
                      </div>
                    )}
                  </td>
...
export default function App() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (saved) {
        ...
        // v38i: MIGRATION OB — chỉ xóa OB schema cũ (thiếu market/factoryId).
        // V38i chuyển sang model TT × NCC: mỗi OB BẮT BUỘC có cả market + factoryId.
  ...
  // v47: Auto sync S3 — Push/Pull mỗi 2 phút + flush khi đóng tab
  ...
  // v38L: Tính tác động công nợ NCC giữa 2 trạng thái data (trước & sau).
  // factoryIds: danh sách NCC cần so sánh.
  // Trả về mảng impact [{ factoryId, factoryName, changed, lines }] — gắn vào detail.debtImpact.
  const computeDebtImpact = (dataBefore, dataAfter, factoryIds) => {
    const settings = data.settings;
    const impacts = [];
    (factoryIds || []).forEach(fid => {
      const before = snapshotFactoryDebt(fid, dataBefore, settings);
      const after = snapshotFactoryDebt(fid, dataAfter, settings);
      const impact = buildDebtImpactLog(fid, before, after, dataAfter.factories || []);
      if (impact) impacts.push(impact);
    });
    return impacts;
  };

  // v38L: Các entity cần ghi tác động công nợ NCC
  const DEBT_IMPACT_KEYS = ["pos", "shipments", "warranties", "payments", "openingBalances"];
  ...
  const onAdd = (key, item) => {
    const dataAfter = { ...data, [key]: [...data[key], item] };
    // v38L: Ghi tác động công nợ NCC (chỉ cho 5 entity liên quan)
    let detail = { ...item };
    if (DEBT_IMPACT_KEYS.includes(key)) {
      // PO mới tạo luôn ở "Chờ duyệt" → chưa ảnh hưởng công nợ → bỏ qua
      const skipPO = key === "pos" && !item.approved;
      if (!skipPO) {
        const factoryIds = getRelatedFactoryIds(key, item, dataAfter);
        const impacts = computeDebtImpact(data, dataAfter, factoryIds);
        if (impacts.length > 0) detail = { ...detail, debtImpact: impacts };
      }
    }
    const newLog = addAuditLog(`create_${actionLabel(key)}`, item.id || item.username, detail);
    save({ ...dataAfter, auditLog: newLog });
  };
  const onEdit = (key, id, updates) => {
    ...
    // v38L: Ghi tác động công nợ NCC
    const dataAfter = { ...data, [key]: data[key].map(x => x.id === id ? { ...x, ...updates } : x) };
    let detail = { ...updates };
    if (DEBT_IMPACT_KEYS.includes(key)) {
      // PO: chỉ ghi tác động khi duyệt (Chờ duyệt → Đã duyệt).
      // Phát hiện duyệt: oldEntity chưa approved, updates.approved = true.
      const isApprovingPO = key === "pos" && !oldEntity?.approved && updates.approved === true;
      const skipPO = key === "pos" && !isApprovingPO;
      if (!skipPO) {
        // NCC liên quan = union(NCC bản cũ, NCC bản mới) — phòng trường hợp sửa đổi NCC
        const oldRecord = oldEntity || {};
        const newRecord = { ...oldEntity, ...updates };
        const factoryIds = Array.from(new Set([
          ...getRelatedFactoryIds(key, oldRecord, data),
          ...getRelatedFactoryIds(key, newRecord, dataAfter),
        ]));
        const impacts = computeDebtImpact(data, dataAfter, factoryIds);
        if (impacts.length > 0) detail = { ...detail, debtImpact: impacts };
      }
    }
    const newLog = addAuditLog(action, id, detail);
    save({ ...dataAfter, auditLog: newLog });
  };
        ...
        <div style={{ width: 240, background: C.sidebar, display: "flex", flexDirection: "column", flexShrink: 0, position: "sticky", top: 0, height: "100vh" }}>
          <div style={{ padding: "24px 22px", borderBottom: "1px solid rgba(255,255,255,0.08)" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <div style={{ width: 42, height: 42, borderRadius: 12, background: `linear-gradient(135deg, ${C.green400} 0%, ${C.green600} 100%)`, display: "flex", alignItems: "center", justifyContent: "center", color: "white", fontSize: 18, fontWeight: 800 }}>GC</div>
              <div>
